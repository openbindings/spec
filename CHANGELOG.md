...
- **Reference resolution (section 10).** Single section establishing that documents are context-free: no `id` field, and every reference (`sources[*].location` and schema `$ref`/`$id`) absolute or same-document, so a document resolves identically wherever it was obtained. Covers reference forms and `$ref` cycle handling.
...
- **Conformance corpus (`conformance/`).** Fixture-based test corpus keyed to rule IDs. 104 tests across 13 document rules and 3 tool rules. Includes `manifest.json`, `fixture.schema.json`, a reference Go runner, plus verification and manifest generation scripts.
