...
- **Example validation strengthened.** `examples[*].input` and `examples[*].output` MUST validate against their operation schemas (OBI-D-12). Was SHOULD in 0.1.0.
...
- **Reference resolution (section 10).** Single section covering document identity (the URI a document is retrieved from; no separate `id` field), `sources[*].location`, and schema `$ref` under one RFC 3986 rule.
- **Versioning (section 11).** Two-axis model: `openbindings` (spec version, SemVer 2.0.0) and `version` (opaque contract version).
...
- **Schema-level enforcement of cross-reference patterns and uniqueness.** Cross-reference values (`bindings[*].operation`, `bindings[*].source`) and `TransformOrRef.$ref` are now pattern-constrained in the JSON Schema. The `aliases` array carries `uniqueItems: true`. These backstop existing prose rules (OBI-D-05, OBI-D-09 through OBI-D-14) with structural validation that catches typos before cross-reference resolution.
...
- **Companion format spec bumped**: `openbindings.operation-graph@0.1.0` to `@0.2.0`. Changes:
  ...
  - **Conformance subcorpus added** under `conformance/operation-graph/`: twelve execution fixtures (one per normative example, plus fixtures exercising the merge-in-cycle lineage rule, `combine` completion-readiness, buffer flush precedence, and empty-buffer completion, with mocked operation responses and expected output streams) and validation fixtures for the well-formedness rules, verified in CI by `scripts/verify-operation-graph.mjs` (which also checks every inline spec example against the op-graph schema). A reference runner (`conformance/operation-graph/runners/js/`) executes the fixtures against a deterministic engine with mocked operations and diffs the output stream, also in CI.
