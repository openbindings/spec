...
**Pending revision against the core draft.** Every candidate here predates two changes in the 0.2 core draft: a source is its `bindingSpec` plus `content` its binding specification defines, with no core `location`; and the core no longer defines transforms, so a binding carries `content` its binding specification defines in place of `selector`, `inputTransform`, and `outputTransform`. Candidate text that relies on the removed members, on the draft's core transform positions or transform-evaluation rule, or on the former OBI-B-02 items describes the earlier draft; the candidates await revision to define source and binding `content`. Citations of the removed core section and of rules the draft retired point at the [changelog](../CHANGELOG.md), and the candidates cite the core's other rules by their current numbers.
...
Several exact upstream patch editions may belong in one revision when they share an upstream line and the artifact identifies its own edition. The accepted editions remain a finite enumeration: a future edition never widens an existing sibling. A family designed to span all upstream versions may omit the upstream-line segment under the naming convention above. No compatibility or range relation between distinct binding-specification identifiers is inferred.
...
1. A specification may incorporate an existing artifact format and protocol. The four `openbindings.openapi-<line>@1` siblings incorporate their named OpenAPI line and HTTP.
...
| graphql         | [openbindings.graphql.md](graphql/openbindings.graphql.md)                         | **unreleased @1 candidate** | `openbindings.graphql@1`         | the operation name of one named operation definition in the carried document; absent for the lone operation |
...
way, in the same place, in the same words** — so that the `openbindings.openapi-<line>` siblings,
...
- **GraphQL:** The operation is a named operation definition in a carried executable document, validated and typed against a carried or live-introspected schema; the caller's input is the variables map and the successful output is the whole `data` map of one execution result. A GraphQL result under §8's classification that carries `data` and `errors` emits `data` and then completes unsuccessfully with the errors as application-authored failure data. Subscriptions are bound on the graphql-ws and graphql-sse transports against an endpoint carried in the source's content, with a configuration point for the disposition of an errored event; the earlier root-field candidate is retired (program record: `audit/graphql-family-2026-09-06`).
...
11. **References** — every normative authority named in the body, including the exact OpenBindings Core version when Core is incorporated.
