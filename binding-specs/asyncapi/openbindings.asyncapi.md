...
**Configuration value shapes.** Each point's `context.configuration` value is pinned so two implementations carry it identically: **`server`** is an object — `{ "key": "<server-name>", "variables": { "<variable-name>": "<string-value>" }? }` selects a member of the effective server set, the optional `variables` member supplying values for the selected server's own declared variables (names are the artifact's declared variable names; values are strings, the Server Variable Object's own value space); xor `{ "url": "<connection-url>" }` overrides with a complete URL whose scheme picks the protocol (an out-of-revision scheme is refused). The two forms are mutually exclusive, and `variables` composes only with `key`. A supplied name the selected server does not declare is refused, never ignored; a supplied value outside the variable's declared `enum` is refused — upstream states SHOULD, hardened to a refusal here, this specification's own pin, per its no-guess posture. The carriage is load-bearing: AsyncAPI, unlike OpenAPI, declares a Server Variable's `default` OPTIONAL, so an undefaulted variable is satisfiable only by a supplied value (the [openapi sibling](../openapi/openbindings.openapi.md#93-servers-and-the-target-url) sanctions the same supply at its server point). **`address`** is an object of parameter values, `{ "<name>": "<value>" }`, supplying the channel address's `{name}` expressions over the declared defaults. **`decode`** is `{ "lane": "json" | "text" }`.
...
- **ASYNC-P-04**: The server, address, and decode configuration points behave per [§9.2](#92-configuration-points) — effective-set order, no-guess address and parameter refusals, supplied-else-default server-variable substitution with its undeclared-name and enum refusals, the concatenation URL-assembly rule.
