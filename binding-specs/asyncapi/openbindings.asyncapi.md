...
| **decode** | the governing declared content types, per direction ([§9.3](#93-decode)) | How each delivery unit becomes an output value. Envelope unwrapping (`{error}`/`{data}` conventions) is a consumer override at this point, never a built-in. |
...
- **ASYNC-D-03**: `ref` is present and is a JSON Pointer `#/operations/<operation-key>` addressing an operations-map entry — the only conformant spelling, RFC 6901-escaped, Reference Objects resolved — per [§7](#7-ref).
