...
The opaque binding-specification identifier is **`openbindings.connect@1`**.
A publication-manifest entry for these exact bytes establishes it under core
[OBI-B-01](../../openbindings.md#104-binding-specification-rules); later
incompatible changes require a different identifier under
...
This specification incorporates exactly version **0.2.0** of the
[OpenBindings Specification](../../openbindings.md) as its Core authority.
Throughout this document, **Core** means that exact version.

This is the **openbindings project's** binding specification for services speaking the **Connect protocol** with the **JSON codec**. It is authored under this project's authority, not by or for the Connect project. Under that authority, this specification deliberately incorporates the Connect protocol to govern routing, framing (unary bodies and enveloped streams), error shapes, compression negotiation, and transport requirements; it incorporates the protobuf language, descriptor model, binary meaning, canonical JSON mapping, and schema projection through [`openbindings.module.protobuf-correspondence@1`](/binding-spec-modules/protobuf-correspondence/1). It chooses close upstream deference rather than treating those authorities as automatically applicable.
...
**Normative upstream snapshot.** The Connect protocol authority is [`src/content/docs/docs/protocol.md`](https://github.com/connectrpc/connectrpc.com/blob/c547a5412ee4f8428f31fce83dd2bb2a82942b76/src/content/docs/docs/protocol.md) at official `connectrpc/connectrpc.com` commit [`c547a5412ee4f8428f31fce83dd2bb2a82942b76`](https://github.com/connectrpc/connectrpc.com/tree/c547a5412ee4f8428f31fce83dd2bb2a82942b76). The rendered protocol page and later repository revisions are informative and do not alter this identifier; advancing the normative pin publishes a new binding-specification identifier. Protobuf authorities retain the exact pins incorporated through `openbindings.module.protobuf-correspondence@1`; Connect's protocol document governs Connect routing, headers, framing, compression, and outcome semantics, while the module's protobuf authorities govern schema and ProtoJSON correspondence.

**Shared schema layer.** This family incorporates the exact companion-module identifier `openbindings.module.protobuf-correspondence@1` and, at publication, records its exact SHA-256. The module supplies descriptor validation, ProtoJSON input/output, binary meaning, and schema projection; it supplies no Connect target, framing, lifecycle, or outcome semantics. This specification deliberately admits only its two locally stated embedded carriers even though another module consumer may expose a different carrier. A later change to the module's observable meaning requires a new module identifier and a new identifier for every affected consumer. One `.proto` can yield both Connect and gRPC bindings — a server speaking both protocols is bound twice, as two sources in one OBI. Three protocol variants are **excluded** from revision 1: the binary proto codec, gRPC-Web, and the protocol's GET dispatch lane for side-effect-free unary methods — every dispatch under this identifier is a POST (the GET lane's message-in-URL carriage brings caching and logging exposure this specification does not weigh). Each exclusion is a definition, not an open item.
...
- **`content` present — schema mode.** The two carriers are defined here and interpreted under `openbindings.module.protobuf-correspondence@1`: single-file proto source text (string, compiled as virtual `source.proto` under the module's exact import allowlist) or strict ProtoJSON for `google.protobuf.FileDescriptorSet` (object). The tagged binary descriptor carrier is not admitted by this family. Acceptance is evaluated per bound-method closure (**CONN-D-01**).
...
A source's `content`, when present, MUST be one of the two **embedded** schema carriages defined in [§3](#3-accepted-source-representations) and interpreted under `openbindings.module.protobuf-correspondence@1` (**CONN-D-01**). No other JSON type or shape is an accepted `content` value under this specification.
...
A binding's `selector` is REQUIRED — this family defines no whole-artifact invocation — and is exactly `<fully-qualified-service>/<method>`, package-qualified or bare for packageless schemas, with both segments nonempty and containing no `/`. It is matched **byte-exactly** against descriptor service and method names in schema mode (**CONN-D-03**).
...
**Schema mode** — the interaction shape is derived from the method descriptor's `client_streaming` and `server_streaming` flags (**CONN-P-04**): both false is unary; only server true is server-streaming; only client true is client-streaming; both true is bidirectional. Bidirectional interaction is full-duplex, with output deliverable while input remains open, and is available only where the transport permits it (the Connect protocol requires HTTP/2). Stream lifecycle — half-close and cancellation propagation — is the **Connect** protocol's, incorporated. An implementation that does not cover a kind declares that as its own limitation and refuses such a method before dispatch — a coverage declaration, never a reinterpretation.
...
This family has **one** named configuration point, **target**. With no effective choice, the source's `location` is the target; an effective consumer choice may supply a replacement base URL in the same form as [§4](#4-location). Where that choice comes from and how configuration scopes are ordered is runtime surface. Nothing else is configurable: routing and framing are protocol-answered, and decode and classification are fixed below.

### 9.2. Schema mode: input and decode

With a schema, value correspondence follows `openbindings.module.protobuf-correspondence@1` rules PB-P-01 through PB-P-05 (**CONN-P-02**): each input value takes the module's portable caller-JSON form — the canonical effective JSON spelling or original Protobuf field name, exact numeric/Base64/map-key forms, canonical well-known-type forms, absent fields as defaults, and **unknown fields refused loudly**. A first input value's failure refuses before dispatch; a later client-streaming or bidirectional value's failure terminates the invocation and cancels the stream, with already-emitted outputs standing. An absent input value marshals as the empty request message for unary or server-streaming interaction, or closes input with zero request messages for client-streaming or bidirectional interaction. Each output value is the response message rendered by the module's canonical ProtoJSON mapping and emitted as frames arrive; a response value that fails to decode against its descriptor — **including one with an unknown JSON member** — is a failure outcome: loud, never silently passed through or dropped. Connect's JSON codec incorporates ProtoJSON, whose parser rejects unknown fields by default; binary Protobuf's unknown-field skipping does not authorize a binding specification to impose that different behavior on a JSON wire.
...
Connect response metadata — unary trailing metadata via the protocol's `trailer-`-prefixed response headers (unary Connect uses no HTTP trailers), streaming trailing metadata in the END_STREAM envelope — has **no representation in the abstract invocation** in revision 1. An artifact runtime or Connect tool may retain it below the OpenBindings bridge.

### 9.5. Classification

Classification is **protocol-native and not a configuration point** (**CONN-P-06**), following the Connect protocol's own rules, incorporated: a **unary** invocation succeeds **iff** the final response status is 200 — the protocol makes every unary error non-200, so this is Connect's rule, not a 2xx heuristic; a **streaming** invocation succeeds **iff** the stream rode HTTP 200, as the protocol requires, and its END_STREAM envelope carries no `error` member. Output values already emitted from a stream **stand**; an END_STREAM error makes the invocation a failure without retracting them. Failure outcomes are not operation results. The abstract invocation reports code-only unsuccessful completion. The service schema does not declare a contract for Connect's error envelope or its detail set, so the Connect error body, code, message, details, HTTP fields, and END_STREAM envelope are not application `data`; artifact runtimes and Connect tooling may retain those facts below the bridge.
...
- **CONN-D-01**: `content`, when present, is one of this specification's two embedded schema carriages interpreted under exact module `openbindings.module.protobuf-correspondence@1`, per [§3](#3-accepted-source-representations) and [§5](#5-content).
...
- **CONN-P-02**: Schema-mode input and decode follow exact module `openbindings.module.protobuf-correspondence@1`, including PB-P-01 through PB-P-05 — portable caller-JSON forms, unknown input or response members refused, absent-input rules, and the first-value-pre-dispatch / later-streaming-value-cancellation refusal split — per [§9.2](#92-schema-mode-input-and-decode).
...
- **[openbindings.module.protobuf-correspondence@1]** [`openbindings.module.protobuf-correspondence@1`](/binding-spec-modules/protobuf-correspondence/1). Incorporated by exact identifier and publication-recorded digest for descriptor and value correspondence ([§2](#2-scope-and-incorporated-authorities), [§3](#3-accepted-source-representations), [§9.2](#92-schema-mode-input-and-decode)).
- **[protobuf] / [ProtoJSON]** As incorporated through `openbindings.module.protobuf-correspondence@1`.
...
- **[OpenBindings Specification 0.2.0]** [OpenBindings Specification 0.2.0](../../openbindings.md). Incorporated as the exact Core authority in [§2](#2-scope-and-incorporated-authorities).
