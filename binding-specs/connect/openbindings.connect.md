...
**Shared schema layer.** This family shares its protobuf schema layer with the `openbindings.grpc@1` candidate, incorporated by **proposed exact-identifier citation**: the embedded schema carriages, their parse pins, and the accepted schema range of `openbindings.grpc@1` [§3](../grpc/openbindings.grpc.md#3-accepted-source-representations) apply here identically, as do its input-correspondence and decode rules where a schema is present ([§9.2](#92-schema-mode-input-and-decode) of this document). Both candidates remain mutable and must be reviewed together before either is published. If published, the exact `@1` citation will freeze that text: a future `openbindings.grpc@2` would not silently alter this specification. One `.proto` can yield both families' bindings — a server speaking both protocols is bound twice, as two sources in one OBI. Three protocol variants are **excluded** from this first candidate: the binary proto codec, gRPC-Web, and the protocol's GET dispatch lane for side-effect-free unary methods — every dispatch under this identifier is a POST (the GET lane's message-in-URL carriage brings caching and logging exposure this candidate does not weigh). Each exclusion is a definition, not an open item.
