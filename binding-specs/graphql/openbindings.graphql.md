...
The binding defines no universal error vocabulary and requires no GraphQL error object, HTTP field, or response envelope on the ordinary boundary. It admits no application-authored failure data: introspection describes field result types, but does not declare a portable contract for GraphQL error entries or their open-ended `extensions`, so an unsuccessful completion is bare under any invocation surface. Treating that protocol envelope as opaque application data would make callers understand GraphQL. An API author who models a domain failure in the selected field's declared result type receives it normally as operation output. Artifact runtimes and GraphQL tooling may retain the complete response and transport evidence below the OpenBindings bridge; the abstract invocation does not carry it. Correct ordinary operation use MUST NOT depend on those native representations.
