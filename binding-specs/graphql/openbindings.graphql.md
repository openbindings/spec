...
**[convention]** A document conforms to **GQL-D-03** when each such source's `document` parses as an executable document under §2 of the governing edition and contains no type-system definition or extension, and its interpreted schema passes §3.2's gate 4: a carried string parses as a `TypeSystemExtensionDocument` under §3.1 of the governing edition, a carried object carries an object at `data.__schema` and no `errors` entry, each string being a `SourceCharacter` sequence, and, with `schema` absent, the live result of §3.3 passes the same test.

**[convention]** A document conforms to **GQL-D-04** when each such source's interpreted schema, carried or live-introspected, satisfies §3.2's schema-validity gate: every referenced type is defined, every field and argument has a type, no two types or directives share a name and no definition or extension repeats a field or argument name, input positions carry input types and output positions output types, at most one `schema` definition exists and any schema extension has one to extend, and the `query` root operation type is present and is an Object type.

**[convention]** GQL-D-03's live clause and GQL-D-04 are decided over the interpreted schema, never over the OBI document's text alone. Where a schema-absent source's live introspection yields no response, each is **unverified** rather than violated, and a verifier reporting an overall conclusion reports conformance undetermined absent an established violation elsewhere; this extends, by this specification's own convention, the treatment Core [§10.5](../../openbindings.md#105-verification-conclusions) gives its own rules over network-inaccessible resources, because an unavailable service is not evidence of a defective document.
