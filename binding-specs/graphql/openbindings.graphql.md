...
**[convention]** Synthesis emits one operation per operation definition accounted `represented` or `lossy`; an `invalid` or `excluded` definition yields a coverage entry and no operation (§3.2). One operation per definition because the operation definition is the unit §6.1 of the governing edition executes. Operation-key spelling, the key derived for a lone anonymous operation, and the resolution of a name collision between two sources under Core [OBI-D-04](../../openbindings.md#102-document-rules) are synthesis policy, not binding meaning, since Core states no key policy and this specification states none; §11.4 declares the freedom and what holds across it. The input schema is §6's, the output schema §7's, and the operation description the executable definition's own description when the governing edition's §2.2 carries one.
...
**[convention]** A document conforms to **GQL-D-03** when each such source's `document` parses as an executable document under §2 of the governing edition and contains no type-system definition or extension, and its `schema`, when present, parses as a `TypeSystemExtensionDocument` under §3.1 of the governing edition when a string, or carries an object at `data.__schema` and no `errors` entry when an object, each string being a `SourceCharacter` sequence (§3.2 gates 3 and 4).

**[convention]** A document conforms to **GQL-D-04** when each such source's interpreted schema, as carried, satisfies §3.2's schema-validity gate: every referenced type is defined, every field and argument has a type, no two types or directives share a name and no definition or extension repeats a field or argument name, input positions carry input types and output positions output types, and the `query` root operation type is present and is an Object type.

**[convention]** A synthesizer conforms to **GQL-S-01** when it emits one operation per operation definition accounted `represented` or `lossy`, emits none for an `invalid` or `excluded` definition, and accounts every operation definition with exactly one status under §3.2's precedence (§11.2).
