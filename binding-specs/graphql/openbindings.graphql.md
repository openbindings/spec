...
**[incorporated]** GQL-D-03's schema clause and GQL-D-04 are decided over carried content only: Core's fifth invariant makes every document rule decidable from the document and locally available resources and never by reaching the network, so for a schema-absent source both are **unverified** rather than violated under Core [§10.5](../../openbindings.md#105-verification-conclusions), whatever the service would disclose and to whomever; the live result's gates are the processor's under GQL-P-01 and GQL-P-03, not a document rule. A verifier reporting an overall conclusion reports conformance undetermined absent an established violation elsewhere.
...
| --- | --- | --- |
| the whitespace of the composed introspection query | §3.3 | the selections and arguments it carries are exactly the listed query's less those the probe did not observe; the probe's bytes are fixed |
