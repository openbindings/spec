...
| --- | --- | --- |
| which credentials and configured fields a synthesizer loads a schema-absent source with, and whether a runtime loads such a source once per invocation or reuses the loaded source across invocations carrying the same credentials | §3.3 | the two introspection requests, their order, their method, and the fields they carry are fixed; the contract emitted for a given credential set is fixed and is a claim about that set (§3.3); a reused schema is subject to §4's staleness posture for the reused interval |
