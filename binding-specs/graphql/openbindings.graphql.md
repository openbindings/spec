...
**[pin]** The graphql-sse protocol defers to the W3C event-stream text through an undated pointer to its latest version. This specification pins that pointer to the Recommendation of 3 February 2015, the W3C's last Recommendation of the format, because an incorporated authority must be immutable at a cited revision; the losing reading, the WHATWG HTML Living Standard's server-sent-events section that the protocol's introduction also links, is disclosed and rejected because it has no fixed revision to cite.
...
**[convention]** Absolute means [RFC 3986 §4.3](https://www.rfc-editor.org/rfc/rfc3986#section-4.3)'s `absolute-URI`, which carries no fragment, here and at `content.subscriptions.location` alike, because Core names the form and states no grammar for it and one grammar must decide the load gate.
