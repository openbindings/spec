...
**[limit]** An `invalid` or `excluded` operation definition is removed from the effective inventory consumed below its owning boundary: no synthesis, dispatch, or classification rule reads it as usable, and synthesis emits no operation for it, only its coverage entry. Removal does not erase its addressable position.
...
**[incorporated]** A string `schema` is read under §3 of the governing edition as a `TypeSystemExtensionDocument`, the §3.1 production whose definitions and extensions together describe one schema. Its interpreted schema comprises what the string writes together with the five built-in scalars (`Int`, `Float`, `String`, `Boolean`, `ID`), of which §3.5 says that in the type system definition language "all built-in scalars must be omitted for brevity", and the built-in directives of §3.13 (`@skip`, `@include`, `@deprecated`, `@specifiedBy`, `@oneOf`), of which §3.13 says "any built-in directive may be omitted for brevity"; gate 5's defined-type test and §5.7.1's directive test read them as defined. The built-ins supplement the string and never duplicate it: a built-in scalar or directive the string itself defines is read as written, in place of the built-in, so gate 5's duplicate-name test does not fire on it and a written definition that differs from the edition's governs validation as written, because the edition permits the omission and states no precedence for a written one. An introspection result carries the built-in directives itself, as §4.2 requires, and the referenced built-in scalars, as §3.5 requires.
...
**[pin]** A selection of the `__schema` or `__type` meta-fields is validated, and its output derived (§7), against the introspection types as §4.2 of the governing edition defines them in type-system language, not against the carried schema's type set. Two readings survive, since a carried string schema never writes the introspection types under any edition and June 2018 does not require them to appear in an introspection result's `__Schema.types` while October 2021 and September 2025 do: validate against the carried set, under which every introspection selection over a string schema or a June 2018 result is invalid, or against §4.2's definitions, under which a selection an earlier service refuses answers with the request error §8's classification already carries. This specification pins the second and discloses the first, because the first invalidates working documents against every string schema and on an accepted edition. `__typename` needs no such rule: §4.1 defines it on every object, interface, and union type of the carried schema, and §7 derives it from that schema.

**[limit]** Validation runs under the September 2025 rules for every service, and the places where that edition is stricter than an earlier accepted edition are the observable consequence of §2's governing-text pin. Each lands on the axis its rule belongs to. Two are parse rules and refuse at gate 3: the lexical lookahead restrictions on numbers (October 2021 §2.9.1 and §2.9.2, under §2's lookahead preamble), and a lone fixed-width Unicode escape that is not a Unicode scalar value and not half of a surrogate pair (September 2025 §2.10.4). One is a schema rule gate 5 enforces: argument names must be unique within a field (September 2025 §3.6). Two are schema rules no rule of this specification reaches, because their violation changes no correspondence: an input object with an unbroken chain of non-null singular self-references (October 2021 §3.10) and input-object default values that form a cycle (September 2025 §3.10) load and dispatch as declared, §6 stating the input schema of such a type, and the service's own answer is the only consequence. Four are document rules, and a document an older service would accept but September 2025 rejects is inventory-invalid under §5.2: an operation whose kind's root operation type the schema does not provide (September 2025 §5.2.1.1, Operation Type Existence, which the earlier editions state nowhere, so this is the one tightening an ordinary document reaches); a subscription's single root field must not be an introspection field (October 2021 §5.2.3.1); `@skip` and `@include` are forbidden on a subscription's root selection set (September 2025 §5.2.4.1); and the `includeDeprecated` argument of `fields` and `enumValues` is non-null in September 2025 §4.2 where the earlier editions declare it nullable, so an explicit `null` there fails §5.6.1 (the argument on `args` and `inputFields` is a September 2025 addition, not a tightening).
...
**[convention]** A document conforms to **GQL-D-05** when every binding whose source names §1's identifier carries either a `selector` that is one `Name` under §2.1.8 of the governing edition naming exactly one operation definition of its source's carried document, or no `selector` over a document holding exactly one operation definition (§5.1).
...
**[convention]** A processor conforms to **GQL-P-05** when it resolves a present `selector` to the operation definition of that exact name and an absent one to the lone operation definition, refuses at resolution a selector that is not a `Name`, names no definition, or names a shared name and an absent selector over zero or several definitions, resolves a selector naming an `invalid` or `excluded` definition and then refuses before dispatch, and carries the selected operation's name as `operationName` when it is named and none for the lone anonymous operation, on every lane (§5.1).
...
| validation runs under the September 2025 rules for every service; the nine tightenings over earlier editions are listed, each on its axis | §5.2 |
