...
**[incorporated]** A string `schema` is read under §3 of the governing edition as a `TypeSystemExtensionDocument`, the §3.1 production whose definitions and extensions together describe one schema.
...
**[convention]** The selected operation's name is what §8 and §9 carry as `operationName`, because the selection must be stated on the wire whenever a name exists, and for the lone anonymous operation no `operationName` is sent on any lane, because omission is the one spelling every lane admits (§8 states both rules with the reason the preserve rung fails and discloses the empty string as the losing reading).
...
**[limit]** Validation runs under the September 2025 rules for every service, and the places where that edition is stricter than an earlier accepted edition are the observable consequence of §2's governing-text pin. Each lands on the axis its rule belongs to. Two are parse rules and refuse at gate 3: the lexical lookahead restrictions on numbers (October 2021 §2.9.1 and §2.9.2, under §2's lookahead preamble), and a fixed-width Unicode escape that is not a Unicode scalar value (September 2025 §2.10.4). One is a schema rule gate 5 enforces: argument names must be unique within a field (September 2025 §3.6). Two are schema rules no rule of this specification reaches, because their violation changes no correspondence: an input object with an unbroken chain of non-null singular self-references (October 2021 §3.10) and input-object default values that form a cycle (September 2025 §3.10) load, derive, and dispatch as declared, and the service's own answer is the only consequence. Three are document rules, and a document an older service would accept but September 2025 rejects is inventory-invalid under §5.2: a subscription's single root field must not be an introspection field (October 2021 §5.2.3.1); `@skip` and `@include` are forbidden on a subscription's root selection set (September 2025 §5.2.4.1); and the `includeDeprecated` argument of `fields`, `enumValues`, and `inputFields` is non-null in September 2025 §4.2 where the earlier editions declare it nullable, so an explicit `null` there fails §5.6.1.
