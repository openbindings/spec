...
- **No `content`** — the schema is obtained live from the addressed server via the gRPC Server Reflection Protocol: the `grpc.reflection.v1.ServerReflection` service, falling back to `v1alpha` **iff** the `v1` info stream fails with status `UNIMPLEMENTED` — any other status is that source's failure, not a version signal (**GRPC-P-01**). A server exposing neither makes a location-only source's bindings unresolvable.
