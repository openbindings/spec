...
The **transport** point's value is one of two shapes. It is the string `"plaintext"` or `"tls"` — an explicit election of channel security that overrides the address-form determination directly. Or it is an object supplying channel security material, whose members are drawn from a closed set: `ca`, `clientCert`, `clientKey`, and `serverName`, each a string. `ca`, `clientCert`, and `clientKey` are PEM-encoded; `serverName` is the hostname to verify against the server certificate. `ca` establishes a custom root of trust; `clientCert` and `clientKey` together present a mutual-TLS identity, and one without the other is not conformant; `serverName` overrides the name checked against the server certificate. A member outside that set is refused loudly, matching this family's input posture ([§9.1](#91-input-mapping)); a member whose value is not a string is refused; and a `transport` value that is neither of the two string elections nor such an object is refused. The **target** point's value is a dial address string in the [§4](#4-location) forms.
