...
Like the core specification, this document defines portable meaning rather than an invocation API. Request names, progress/result ordering, and refusal conditions describe the MCP interaction denoted by a binding; session objects, client methods, retries, context negotiation, and pooling remain runtime or SDK surface.
...
Each entity family denotes its corresponding MCP request and carries its own interaction shape:
...
One action through the binding is one MCP request (plus the notifications MCP attaches to it), after the required-task refusal above. Progress solicitation applies to tools only: implementations do not solicit or surface progress for `resources/read` or `prompts/get` in revision 1. Task augmentation and server-initiated MCP features — subscriptions, sampling, elicitation, roots, and log streams — are **excluded** from revision 1's binding surface. Each exclusion is a definition, not an open item.
...
- **Solicitation is a named configuration point** ([§9.3](#93-configuration-and-result-correspondence)), preserving both protocol-permitted choices. With no effective choice, the convention is **not solicited**: no `progressToken` is attached, and the stream is the result value alone. This is explicitly an **OpenBindings convention**, reached only after the protocol's alternatives have been exposed as configuration; a determinate stream shape requires a choice, and the no-token alternative adds no protocol traffic or extra operation values. Consumers that want progress opt in knowingly.
...
This family has **one** named configuration point. The table defines the semantic effect of its **effective** choice and the convention when none is supplied; configuration scopes and their precedence are runtime surface. Classification is not configurable here ([§9.4](#94-classification)). The convention is **content-independent** — decided by protocol structure, never payload bytes (**MCP-P-05**):
...
Classification is **protocol-native and not a configuration point** (**MCP-P-06**): a `tools/call` result with `isError: true` is a failure outcome, whatever its content; a JSON-RPC error response is a failure outcome; every other completed result is the success correspondence above. Failure outcomes are not operation results. The invocation terminates as a failure while preserving the complete native evidence: the full `CallToolResult` for `isError`, JSON-RPC `code`, `message`, and `data` for an error response, and the Streamable HTTP status, fields, and exact entity bytes for an HTTP failure. No MCP error is projected into an operation output, and this binding specification defines no replacement error-code vocabulary; a consumer may additionally classify the terminal failure into its own portable taxonomy without discarding that evidence.
