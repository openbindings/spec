...
**[convention]** Multipart part order across different parameters has no portable meaning; repeated parts for one array preserve member order, and part boundaries MUST NOT occur in content.
...
**[incorporated]** Root `consumes` and `produces` provide operation defaults; a present Operation list replaces its root list, including an empty list ([OAS 2.0 Swagger and Operation Objects](https://spec.openapis.org/oas/v2.0.html#fixed-fields-4)).
...
**[pin]** "Media-defined comparison rules" would otherwise send this binding to each media type's live IANA registration, which §9.2 refuses to consult for the same reason. This specification pins the comparison instead: a parameter value is first unquoted under RFC 9110 §5.6.6, so a token and its quoted-string spelling of the same characters are equal; the values of `charset` and `boundary` then compare ASCII case-insensitively, and every other parameter value compares by exact character sequence. No registration lookup changes this. Only the `charset` half is incorporated, and the rest is disclosed here rather than left to the citation: RFC 2046 §4.1.2 says that "the values of the charset parameter are NOT case sensitive" and speaks of nothing else, and no section of RFC 2046 states a comparison rule for a declared `boundary` value. The `boundary` half is therefore this specification's own choice, and the reading it displaces is RFC 2046 §5.1.1's "NOTE TO IMPLEMENTORS: Boundary string comparisons must compare the boundary value with the beginning of each candidate line", which requires the boundary to appear in its entirety and is therefore byte-exact. That note governs finding a delimiter inside a received multipart body — a receiver's operation this specification never performs, because §9.2 makes its multipart lane request-only — and not the identity of two declared media types, which is the only comparison this rule decides. The cost of the difference is stated rather than hidden: two `boundary` spellings this rule makes one identity would name two different delimiters. The `boundary` choice reopens only if an incorporated authority states a comparison rule for a declared `boundary` value ([RFC 9110 §§5.6.6, 8.3.1](https://www.rfc-editor.org/rfc/rfc9110#section-5.6.6), [RFC 2046 §§4.1.2, 5.1.1](https://www.rfc-editor.org/rfc/rfc2046#section-4.1.2)).
...
**[limit]** Distinct list entries that normalize to one parsed media identity collide only for that identity and support no selection through it: a request selection refuses before dispatch and a response selection is a loud protocol error. Non-colliding entries survive, and list order never breaks the tie.
...
**[pin]** That rule departs from RFC 7303 §3.2 on both of its clauses, and the departure is disclosed here in the same terms as the RFC 2046 displacement below rather than presented as a reading of the section. RFC 7303 §3.2 states an unambiguous precedence — a BOM is authoritative if present in an XML MIME entity, and in the absence of a BOM the charset parameter is authoritative if present — and it advises that "XML-unaware MIME consumers SHOULD NOT assume a default encoding" where neither is present, an advisory §8.5 repeats. This specification inverts the first and declines the second: a BOM is a body byte, and a binding that decides no other lane by inspecting body bytes will not make an exception for this one, while refusing every charset-less XML entity would leave a large, ordinary, and in practice UTF-8 population of artifacts undecodable for a hazard the charset parameter already exists to signal. A caller or receiver that needs BOM-authoritative decoding cannot obtain it under this identifier; the departure reopens only under a new identifier ([RFC 7303 §§3.2, 8.5](https://www.rfc-editor.org/rfc/rfc7303#section-3.2)).
...
**[convention]** An unmatched, ambiguous, normalized-colliding, or absent effective response media declaration is a loud protocol error; no bytes are sniffed and no undeclared fallback is invented.
...
**[convention]** A synthesizer MUST account for every addressable operation as represented, excluded with the exact reason stated beside the applicable exclusion, or unsupported; a failure in an unused description position is coverage loss rather than invocation behavior.
