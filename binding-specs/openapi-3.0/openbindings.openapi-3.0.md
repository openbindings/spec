...
**[limit]** Omitting `responses` or providing a present empty Responses Object is upstream-invalid and removes only the selected operation under §3.2's smallest-owner rule, accounted `invalid`. The confinement reopens only if an incorporated OAS 3.0 edition admits the exact declaration.
...
**[exclusion]** Two governing response Header Object keys that differ only by ASCII case exclude the smallest owning response alternative before any actual response is inspected, because OAS Header Object keys are case-sensitive while HTTP field names are not and the wire cannot preserve the distinction. The rule reaches every governing response Header Object key, not only the content-coding surface, since a Header Object on this line also carries `required` and a case-distinct pair would leave §9.5's required-presence check with no determinate subject; it mirrors §7's request-side exclusion and reopens only if an incorporated authority defines a wire mapping that preserves such declarations ([OAS 3.0.4 §§3.8, 4.7.17, 4.7.21](https://spec.openapis.org/oas/v3.0.4.html#response-headers), [RFC 9110 §5.1](https://www.rfc-editor.org/rfc/rfc9110#section-5.1)).
...
**[exclusion]** OAS 3.0.4 requires a Server Variable `default` and sends it when no alternate is supplied, but only recommends that an `enum` be nonempty and that the default belong to it. For cross-line determinism this specification removes three declarations from the accepted surface — an empty `enum`, an out-of-enum default, and an otherwise unresolved variable — and rejects one caller value, an out-of-enum substitution; all four converge on one outcome: the selected target is unusable and the invocation **refuses before dispatch** at the affected Server Object. That outcome is a refusal and not §3.2's `excludes`, because a `server` configuration point or a complete configured URL can still discharge the invocation, so the target stays selectable and represented. Refusing the empty-enum-plus-default case is a disclosed narrowing of the authority's SHALL-send-default instruction; the exclusion belongs to the Server Object that owns the variable and reopens only if an incorporated OAS edition defines the excluded declarations' unique substitution ([OAS 3.0.4 §4.7.6](https://spec.openapis.org/oas/v3.0.4.html#server-variable-object)).
...
**[exclusion]** A completed target whose scheme is not `http` or `https` refuses before dispatch, because no incorporated authority defines that scheme's HTTP-semantics mapping. The outcome is a refusal rather than §3.2's `excludes`, as for the Server Variable rule above: the scheme belongs to the completed target, which a `server` choice or a complete configured URL can still change, so the target stays selectable and represented. The exclusion belongs to the Server Object or configured URL that supplied the scheme and reopens only if an incorporated authority defines that mapping.
...
| §9.5 | the upstream-invalid Response Object removal reaches only a declaration that can govern a successful response |
| §9.5 | a governing Response Object that omits `description` while declaring no content does not remove the target |
...
| §9.5 | a target with a Responses key outside the closed admitted set: an upstream-invalid declaration confined to its stated owner and accounted invalid; reopens only if an incorporated OAS 3.0 edition admits that exact key form |
| §9.5 | a target with an upstream-invalid Response Object able to govern a successful response: an upstream-invalid declaration confined to its stated owner and accounted invalid; reopens only if an incorporated OAS 3.0 edition admits the exact declaration |
