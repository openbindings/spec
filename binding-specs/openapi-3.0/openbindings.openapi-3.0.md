...
| 5 — the syntax and meaning of `selector`, including the absent-`selector` case | §3.2, §5.1, §6.1, §12.3 (**OAPI30-D-02**, the only rule stating a consequence for an absent, malformed, or non-resolving `selector`, and decided over the interpreted artifact) | §3.2's outcome vocabulary names no member for a selector that does not resolve. §6.1 states that disposition in the words §3.2 binds to an addressable target that is unusable, and a selector that does not resolve reaches no addressable target at all. |
