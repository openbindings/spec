...
**[convention]** A `boundary` parameter carried by a declaration key or a `requestMedia` choice takes part in §9.1's matching and is then discarded for emission; the token this binding generates is the one emitted and the one that delimits. No incorporated authority makes a declared `boundary` bind the sender — RFC 2046 §5.1.1 leaves the value for the composer to choose and RFC 7578 §4.1 has the sender supply it — and honoring a declared one would make load-bearing a value the rule above declares free.
