...
**[pin]** Key resolution follows that qualifier, and differs deliberately from the value resolution the preceding rule pins: the Failsafe ruleset's only scalar tag is `tag:yaml.org,2002:str`, so every scalar key satisfies the qualifier, a plain scalar key never refuses on its resolved type, and an unquoted `200:` Responses key is the string `200`. The edition's representation rule reaches the same result independently — an OpenAPI Document "is itself a JSON object, which may be represented either in JSON or YAML format", so a mapping key is the representation of a JSON member name, which is a string whatever tag resolution would assign the same characters in value position. What refuses at the key gate is a key that is not a scalar at all — a sequence or a mapping in key position — which no representation makes a member name. The asymmetry between key and value resolution is the incorporated edition's and not this specification's: the edition states the key rule normatively, and this specification does not invent a preference against an authority that speaks. What the edition does not state is the consequence of violating it, which is pinned here ([YAML 1.2.2 §10.1.1](https://yaml.org/spec/1.2.2/#1011-tags), [OAS 3.1.2 §4.2](https://spec.openapis.org/oas/v3.1.2.html#format)).
...
**[convention]** No source mode this specification governs forbids `content`, so the set of such modes is empty. Core's at-least-one-of rule admits exactly three — `location` alone, `content` alone, and both co-present — and the location-only mode is the one in which `content` is absent rather than prohibited; the acceptance rule above is unconditional over all three, and §3.1 and §4 state a meaning for `content` in each mode that carries it. This states the negative Core [OBI-B-02](../../openbindings.md#104-binding-specification-rules) item 3 asks for rather than leaving a reader to infer it, and it moves no observable: it admits no value the rule above refuses and refuses none it admits (Core [§5.4](../../openbindings.md#54-sources)).
...
**[incorporated]** `requestBody` is fully supported in HTTP methods where "the HTTP 1.1 specification [RFC7231] Section 4.3.1 has explicitly defined semantics for request bodies"; "in other cases where the HTTP spec is vague (such as GET, HEAD and DELETE), `requestBody` is permitted but does not have well-defined semantics and SHOULD be avoided if possible." The edition's own pointer for that test is [RFC 7231 §4.3.1](https://www.rfc-editor.org/rfc/rfc7231#section-4.3.1), carried here rather than paraphrased away because the rule below re-bases it. GET, HEAD and DELETE are given as examples of that class, not as its extent, and the edition names no other method ([OAS 3.1.2 §4.8.10.1](https://spec.openapis.org/oas/v3.1.2.html#operation-request-body)).
...
**[incorporated]** A Schema Object `default` documents what the receiver assumes when a value is not provided; it is never inserted by this binding. An unsupplied optional parameter carrying a `default` is omitted before serialization, exactly as one carrying none. The governing text states the distinction while defining the Server Variable Object's own `default`: "Note that this behavior is different from the Schema Object's `default` keyword, which documents the receiver's behavior rather than inserting the value into the data." 3.1.0 carries no such note, and §2's unconditional pin carries the governing text to every accepted edition ([OAS 3.1.2 §4.8.6.1](https://spec.openapis.org/oas/v3.1.2.html#server-variable-object)).
...
**[incorporated]** Header `schema` serialization performs no URI percent-encoding and adds no automatic quotes: the governing text requires that URI percent-encoding MUST NOT be applied to header values and that they pass through unchanged, a correction the governing edition states expressly. Its Appendix D.1 note records that "OAS v3.0.4 and v3.1.1 applied the advice in this section to avoid RFC6570-style serialization to both headers and cookies" and that "this section has been corrected to apply only to cookies". Of the editions this specification accepts only 3.1.1 gave that advice — 3.1.0 carries no Appendix D at all — and neither of them states the percent-encoding requirement itself, so §2's unconditional pin is what carries the corrected rule to all three ([OAS 3.1.2 §4.8.12.2.2](https://spec.openapis.org/oas/v3.1.2.html#fixed-fields-for-use-with-schema), [Appendix D.1](https://spec.openapis.org/oas/v3.1.2.html#appendix-d-serializing-headers-and-cookies)).
...
**[convention]** A supplied request value at a raw-octet boundary that is not a JSON string, or that is a JSON string failing that canonicality in any respect, refuses before dispatch at the affected body or part: the decode is never partially applied, no non-alphabet character is skipped, and no repair supplies missing padding. §3.2's vocabulary reserves a loud protocol error for a wire fact and no wire fact exists at this point, so no other outcome is available. The response direction has no counterpart because encoding the exact octets always succeeds.
...
**[convention]** A `boundary` parameter carried by a declaration key or a `requestMedia` choice takes part in §9.1's matching and is then discarded for emission; the token this binding generates is the one emitted and the one that delimits. This is the one exception to §9.1's emitted-spelling pin, which otherwise emits each parameter value in the characters the declaration or choice supplied. No incorporated authority makes a declared `boundary` bind the sender — RFC 2046 §5.1.1 leaves the value for the composer to choose and RFC 7578 §4.1 has the sender supply it — and honoring a declared one would make load-bearing a value the rule above declares free.
