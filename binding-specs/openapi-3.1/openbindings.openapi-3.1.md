...
**[convention]** A body-emitting invocation emits a request `Content-Type` field carrying the concrete media type elected under §9.1; no declaration key, example, or supplied body value substitutes another media type, and beyond a `multipart/form-data` election's `boundary` parameter — which the incorporated authority supplies as a parameter of that media type — this specification adds no parameter of its own to the emitted value. No accepted OAS edition states this emission: the edition's only statement about the request `Content-Type` field is that a Header parameter of that name is ignored, which §8.1 already carries, so the emission is authored here rather than incorporated ([RFC 7578 §4.1](https://www.rfc-editor.org/rfc/rfc7578#section-4.1), [OAS 3.1.2 §4.8.12.2.1](https://spec.openapis.org/oas/v3.1.2.html#common-fixed-fields)).
