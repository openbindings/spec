...
**[convention]** OBI-B-02 is a floor, not a partition. §11's credential-construction rules supply supporting wire detail for the selected interaction: how credentials are carried, which destinations may coexist, and which transport prerequisites apply. Credential values remain distinct from caller-facing input and output values. These binding rules supplement the item map; they introduce neither a new Core completeness item nor a reporting interface.
