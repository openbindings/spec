...
**[pin]** Key resolution follows that qualifier, and differs deliberately from the value resolution the preceding rule pins: under the Failsafe ruleset every plain scalar resolves to a string, so a plain scalar key never refuses on its resolved type, and an unquoted `200:` Responses key is the string `200`. What refuses at the key gate is a key that is not a scalar at all — a sequence or a mapping in key position — which is no ruleset's scalar string. The asymmetry between key and value resolution is the incorporated edition's and not this specification's: the edition states the key rule normatively, and this specification does not invent a preference against an authority that speaks. What the edition does not state is the consequence of violating it, which is pinned here ([YAML 1.2.2 §10.1.2](https://yaml.org/spec/1.2.2/#1012-tag-resolution), [OAS 3.1.2 §4.2](https://spec.openapis.org/oas/v3.1.2.html#format)).
