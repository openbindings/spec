...
| 5 — the syntax and meaning of `selector`, including the absent-`selector` case | §3.2, §5.1, §6.1, §12.3 | §3.2's outcome vocabulary names no member for a selector that does not resolve; §6.1 states that disposition in the words §3.2 binds to an addressable target that is unusable, and §3.2's own excluded-unit rule separates the two by saying that a selector naming an excluded target still resolves. The absent case itself is reached: §6.1's REQUIRED together with OAPI31-D-02, which is where its consequence is stated. |
| 6 — how the binding target and its interaction are identified | §3.2, §4, §5.1, §5.2, §6.1, §6.2, §7, §8.1, §8.2, §8.3, §9.5, §10, §11, §12.1, §12.3 | None. |
| 7 — how caller-facing input and successful output values correspond to the source interaction, which outcomes are successes, when the interaction instead completes unsuccessfully, how values emitted before that completion are treated, and any context bindings at transform positions | §3.2, §4, §5.1, §5.2, §7, §8.1, §8.2, §8.3, §9.1, §9.2, §9.3, §9.4, §9.5, §11, §12.1, §12.2 | Three conditions, the context-bindings clause being reached by exactly one rule, in §12.2. First, an actual response carrying more than one `Content-Type` field: §9.5 fixes the field's absence and its use in selection, and §9.1 fixes how one field's value is parsed and compared, but no rule here says which value governs when the field is repeated, or that the repetition is itself a loud protocol error. Second, a schema `default` on a parameter the caller does not supply: no rule here says whether the declared value is inserted or the parameter omitted. Third, whether the media-type parameters an elected declaration key or `requestMedia` choice carries ride on the emitted request `Content-Type`: §9.1 fixes only that this specification adds no parameter of its own beyond a `multipart/form-data` election's `boundary`, which settles what the specification supplies rather than what the elected media type carries. |
