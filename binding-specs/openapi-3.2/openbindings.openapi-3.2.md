...
**[pin]** Repeated pairs or parts produced by one array property preserve that array's item order, and only order across distinct properties has no portable meaning. The authority's latitude here is wider than that and is not the source of this rule: OAS 3.2.0 §4.14.5.1 states without qualification that "The order of these name-value pairs in the target media type is implementation-defined", which read literally would leave one array property's own items free to be reordered. This specification pins that latitude to the cross-property case alone, because an array property's item order is the supplied caller value's own order and reordering it would change a value rather than a layout; the narrowing is this specification's own and is disclosed here. Read the sibling lines' `convention` label for the same rule as an edition difference, never as drift: no accepted 3.0 or 3.1 edition states any order sentence, so on those lines no authority speaks at all ([OAS 3.2.0 §4.14.5.1](https://spec.openapis.org/oas/v3.2.0.html#encoding-by-name), against [OAS 3.0.4 §4.7.15.3](https://spec.openapis.org/oas/v3.0.4.html#encoding-multipart-media-types) and [OAS 3.1.2 §4.8.15.3](https://spec.openapis.org/oas/v3.1.2.html#encoding-multipart-media-types)).
...
**[convention]** A `boundary` parameter carried by a content-map declaration key or by a `requestMedia` choice takes part in §9.1's matching and is then discarded for emission: the token the processor generates above is the one emitted on the request `Content-Type` and the one that delimits. This is the single exception §9.1's emitted-spelling pin names. No incorporated authority makes a declared `boundary` bind the sender — [RFC 2046 §5.1.1](https://www.rfc-editor.org/rfc/rfc2046#section-5.1.1) gives the choice to the composer, which "must exercise care to choose a unique boundary parameter value", while [RFC 7578 §4.1](https://www.rfc-editor.org/rfc/rfc7578#section-4.1) states only that the boundary "is supplied as a `boundary` parameter to the multipart/form-data type" and names no declaration as its source — and honouring a declared one would make a value the preceding rule declares free into a load-bearing input.

**[incorporated]** In the receive direction the same framing is read under the incorporated authority's own receiver clauses: a boundary delimiter MUST occur at the beginning of a line, that is following a CRLF, and may be followed by zero or more characters of linear whitespace; a boundary comparison compares the boundary value with the beginning of each candidate line, an exact match of the entire candidate line not being required, so long as the boundary appears in its entirety following the CRLF; and anything appearing before the first boundary delimiter line or after the last one is ignored. This direction is reached because the response exclusion below is scoped to the name-based lane while positional `multipart` is admitted on responses under §9.5 ([RFC 2046 §5.1.1](https://www.rfc-editor.org/rfc/rfc2046#section-5.1.1), [RFC 9110 §8.3.3](https://www.rfc-editor.org/rfc/rfc9110#section-8.3.3)).
...
**[limit]** A runtime MAY impose a resource bound on the size of a single delivered sequential item, as [RFC 9110 §17.5](https://www.rfc-editor.org/rfc/rfc9110#section-17.5) advises recipients to do and Core [§9](../../openbindings.md#9-security-considerations) leaves to the processor. This specification names no such bound, supplies no configuration point for one, and fixes neither its value nor its unit; a bound is a property of the runtime and never of the artifact, so it can reach no coverage entry. Its only binding-visible consequence is fixed: an item the bound rejects is not emitted and the interaction completes unsuccessfully, and the rule above governs the values already emitted. A bound is not latitude inside the decode chain — it selects no coding, decoding, or lane and changes no value — so §12.1's closure is unaffected.
