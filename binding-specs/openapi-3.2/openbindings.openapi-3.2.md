...
**[limit]** Omission of `description` is not a violation on this line, and the difference from the siblings is the authority's: OAS 3.2.0 drops the REQUIRED marker OAS 3.0.4 and OAS 3.1.2 carry on the Response Object's `description` and adds an optional `summary`, so a governing Response Object that omits either or both governs normally, with or without declared content, where the same omission excludes on the 3.0 and 3.1 lines — an edition difference, never sibling drift. `description` is still typed `string`, so one present with a non-string value is a fixed-field violation and excludes as above ([OAS 3.2.0 §4.17.1](https://spec.openapis.org/oas/v3.2.0.html#fixed-fields-14), against [OAS 3.0.4 §§4.7.16, 4.7.17](https://spec.openapis.org/oas/v3.0.4.html#response-object) and [OAS 3.1.2 §4.8.17.1](https://spec.openapis.org/oas/v3.1.2.html#response-object)).
