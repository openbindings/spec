...
**[pin]** A `Content-Encoding` field value on either surface denotes the RFC 9110 §8.4 list of content-coding tokens in wire order: the serialized field value is split on `,`, each element stripped of leading and trailing whitespace, and each token matched ASCII case-insensitively against the configured map keys; a caller-supplied value is serialized under §8's parameter rules first, so one string and one single-element array denote the same one-token list. The admission test below runs separately against the **complete serialized field value**, whether the governing declaration is `schema`-form or `content`-form, because a Header Object describes one field value rather than a token ([RFC 9110 §8.4](https://www.rfc-editor.org/rfc/rfc9110#section-8.4)).
...
**[pin]** "Appended" is pinned here to *verbatim*: no slash normalization and no path repair either. The edition forbids only relative resolution, so this is an entailment stated as this specification's own, with its consequence: a Server URL ending in `/` and a Paths key beginning with `/`, the default server `/` included, produce a doubled slash in the completed target, and this rule forbids repairing it, because a processor that silently rewrites the constructed path can address a resource the artifact did not declare.
