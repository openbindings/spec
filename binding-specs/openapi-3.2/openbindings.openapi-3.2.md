...
| 3 — `content` | §3.1 (the two accepted representations and their meaning), §4 (a present `content` MUST be one of them and no other JSON type is accepted) | Core's forbidden-`content` clause is answered in neither direction: no governed source mode is named as forbidding `content`, and the negative is nowhere stated. §4's unconditional rule entails the empty enumeration without asserting it. |
...
**[convention]** A body-emitting invocation emits the concrete media type elected above as the request `Content-Type` field value, and where a declared range was instantiated by a `requestMedia` choice the emitted value is that concrete choice, because a range is not a media type a receiver can be sent and no other value is available. No accepted edition states this emission: OAS's only statement about a request `Content-Type` is that a Header parameter of that name MUST be ignored, so the emitted field is this specification's own and §8.1's ignore rule leaves no artifact-declared parameter competing for it. A multipart election additionally carries §9.3's `boundary` parameter on this field ([OAS 3.2.0 §4.12.2.1](https://spec.openapis.org/oas/v3.2.0.html#parameter-name)).
