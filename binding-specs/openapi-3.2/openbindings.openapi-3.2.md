...
**[pin]** That `name` parameter value is emitted as a quoted-string over the exact schema-property-name Unicode scalar sequence encoded as UTF-8. A `qdtext` byte is emitted literally, each DQUOTE or backslash is preceded by exactly one backslash, and no other byte is escaped or percent-encoded. [RFC 9110 §5.6.6](https://www.rfc-editor.org/rfc/rfc9110#section-5.6.6) permits quoted and unquoted spellings but fixes neither choice nor quoted-pair use; this specification's single spelling closes those bytes. A governing Encoding Header Object may replace that spelling only when it fixes one valid `Content-Disposition` field whose parsed disposition type is `form-data` and whose `name` parameter equals the exact schema-property name; any other fixed value excludes the name-based multipart alternative.
...
| cross-property pair order in form-urlencoded and name-based multipart bodies | §9.3 | every declared property's own name and value bytes — up to the JSON-image latitude above where a property rides as a JSON image — and, for an array property, its items in order under one repeated name |
...
| --- | --- |
| §3.2 | exactly §5.2's root-dialect exclusion exists; no other source member or addressable target is filtered merely by its position in the source |
...
| §9.2 | JSON-lane numbers are interoperable within RFC 8259 §6's binary64 expectation, over the two-member permitted set indexed in the first table above |
| §9.2 | UTF-8 encoding and decoding are required; every further charset direction is an independent implementation capability |
| §9.3 | an elided null on an optional content-based form property identifies `{}` and `{"x": null}` on the wire |
| §9.3 | non-ignored Encoding `headers` produce no caller channel: only an artifact-fixed value is emitted, and the `contentEncoding`-declared equivalent `Content-Transfer-Encoding` is declaration semantics that emits no field |
...
| §10 | a completed non-HTTP target remains addressable and represented, its invocation refuses before dispatch, and a complete configured HTTP(S) URL remains the recovery; no declaration is excluded |
...
| §6.1 | a present Responses Object with neither a patterned response-code field nor `default` — an empty or extension-only object included: an upstream-invalid declaration confined to the selected target and accounted invalid; reopens only if an incorporated OAS edition admits it |
...
| §8.2 | path-key ambiguity, either direction of a path-expression/parameter mismatch, or repetition of an expression: an upstream-invalid declaration confined to the selected target and accounted invalid; reopens only if incorporated authority admits the declaration or defines its unique target mapping |
...
| §9.3 | a name-based/positional Encoding mutual-exclusion or positional-prerequisite violation: an upstream-invalid declaration confined to the selected media alternative and accounted invalid; reopens only if an incorporated OAS edition admits the declaration and defines its encoding behavior |
| §9.6 | a Responses key outside the closed admitted set: subordinate invalid coverage ignored for lookup; if no admitted response declaration survives in a present Responses Object, the enclosing target is invalid; reopens only if an incorporated OAS 3.2 edition admits that exact key form |
| §9.6 | an admitted response key carrying an upstream-invalid Response value or fixed member: invalidity confined to the smallest response alternative or projection, without changing lookup precedence or target addressability; reopens only if an incorporated OAS 3.2 edition admits the exact declaration |
| §5.1 | a reference reaching a document with neither admitted root, using a retrieval alias despite `$self`, or crossing a nearer `$id` boundary noncanonically: an upstream-invalid declaration confined to the smallest referencing unit and accounted invalid; reopens only if incorporated authority admits the exact form |
| §10 | a malformed Server Variable Object, or a Server URL template repeating a variable: an upstream-invalid declaration confined to that Server alternative and accounted invalid; reopens only if an incorporated OAS edition admits the declaration or defines repeated-variable substitution |
| §10 | a declaration-derived Server expansion containing a query or fragment: an upstream-invalid declaration confined to that Server alternative and accounted invalid; a consumer substitution is only a nonconforming invocation choice; reopens only if an incorporated OAS edition defines the exact cell |
...
Everything removed from the accepted domain, with the owner each removal confines to and the specifically stated condition that would reopen it. §12.3 states that every one of these is permanent under this identifier and promises no later work:
...
| §5.2 | `$dynamicRef` where a decision requires runtime dynamic-scope evaluation | the smallest schema-inspected media alternative, parameter, or multipart property; type-uninspected JSON carriage remains admitted | demonstrated consumer need for portable binding-governed dynamic-scope evaluation; any semantics-changing addition remains subject to OBI-B-03 |
| §6.1 | exact uppercase `CONNECT` | the addressable additional-operation target; a case-distinct token such as `connect` remains admitted | demonstrated consumer need for a tunnel interaction at the OpenBindings operation boundary; any semantics-changing addition remains subject to OBI-B-03 |
...
| §8.3 | a non-token effective header parameter name | that parameter projection when optional; the selected target when required | incorporated HTTP authority admits that exact field-name form |
| §8.3 | a non-token effective cookie parameter name | that parameter projection when optional; the selected target when required | incorporated cookie authority admits that exact cookie-name form |
| §8.3 | an effective header parameter named `Host`, `Content-Length`, `Connection`, `Keep-Alive`, `Proxy-Authorization`, `Proxy-Connection`, `TE`, `Trailer`, `Transfer-Encoding`, or `Upgrade` | the selected target | an incorporated HTTP authority defines caller control preserving the processor's connection, framing, and routing obligations |
| §8.3 | required opposite-kind raw and structured Cookie parameters, or an unavoidable credential/required-parameter Cookie mix | the target for the required-parameter pair; otherwise the owning security alternative | incorporated authority defines a coherent raw/structured Cookie merge |
...
| §9.3 | a fixed name-based `Content-Disposition` value that is unsafe, is not `form-data`, or names a different property | that multipart media alternative | an incorporated authority defines a different unambiguous property-to-part mapping |
| §9.3 | an Encoding-header group with an invalid name, unsafe value, conflicting fixed values, a fixed-value rejection, or required status without one exact value | that multipart media alternative | an incorporated authority admits the field form or defines a different safe composition or caller part-header carriage |
...
| §9.4 | a non-token response Header Object key | that header projection when optional; the smallest owning response alternative when `required: true` | incorporated HTTP authority admits that exact field-name form |
| §9.4 | a required request `Content-Encoding` parameter after no surviving request lane can emit a representation | the selected target | incorporated HTTP authority defines Content-Encoding on a request with no representation |
| §9.4 | a required response `Content-Encoding` case-group whose binding-understood finite raw-string domains have an empty intersection | that response alternative | incorporated OAS authority defines a different co-present constraint composition |
...
| §9.6 | a response-body projection that can govern only HEAD, `1xx`, `204`, `205`, or `304` | that subordinate projection; the operation remains represented | incorporated HTTP authority permits content for that case |
| §10 | a Paths key containing a literal `?` or `#` | the targets beneath that key | incorporated OAS authority defines that key's target mapping |
| §10 | a declaration-derived absolute HTTP Server alternative with an empty host or userinfo | that Server alternative | incorporated HTTP authority permits such target generation |
...
| §11 | an `apiKey` alternative whose header name is not an HTTP `token` or whose cookie name is not a cookie-name `token` | that security alternative | incorporated authority admits the exact destination-name form |
| §11 | two credentials sharing one destination, a credential colliding with a required fixed-destination parameter, or a credential targeting a binding/processor-owned field | the owning security alternative; the target only when no complete alternative remains | incorporated authority defines a safe, unambiguous assembly for the exact collision |
| §11 | a query credential colliding with a required whole-querystring parameter | the owning security alternative | an incorporated OAS edition defines that assembly |
| §11 | a TRACE security alternative that emits credentials, or a TRACE target with a required Cookie parameter | the owning alternative or target | incorporated HTTP authority permits sensitive fields on TRACE |
