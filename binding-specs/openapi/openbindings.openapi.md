...
- A field matching no declared parameter or body property **passes through into the body** only where the selected request media ([§9.2](#92-request-and-response-media)) gives it a destination **without invention** — the **JSON family** (`application/json`, any `+json` suffix), where an extra field is simply another body property. In every other case a field matching neither a declared parameter nor a declared body property is **refused before dispatch** — loudly, never silently dropped: when no request body is declared; when the declared body is **non-object** (the flattened contract carries the synthetic `body` property of the next rule — a string, binary, array, any non-object shape), so there is no property surface to join; and when the operation's object body declares **no** JSON-family request media, so selection lands on `multipart/form-data` or `application/x-www-form-urlencoded`, whose OAS serialization derives each part's carriage from per-property declarations an unmatched field lacks (the same absence the degenerate media/schema refusal turns on, [§9.2](#92-request-and-response-media)). All three are the same species of refusal as the unflattenable cross-location collision above and the missing-required-path-parameter rule below. The rule is stated as a property, not a format allowance: the operation value domain is JSON (core [§5](../../openbindings.md#5-document-model)), so a JSON body needs no translation step for an extra field, and a future request media whose serialization is derivable without a per-property declaration would qualify by the same test. Where passthrough holds, the field **joins the body value before request-media selection completes**: it is a body property like any declared one, riding the JSON body's encoding, never a side channel. No schema evaluation participates in this routing; enforcing the body schema is the server's business.
- A non-object request body schema is represented in the flattened contract under the synthetic **`body`** property; at the wire, that property's value **is** the request body, unwrapped. Object-or-not is decided by **declaration alone**: a body schema is object for this section iff it declares `properties` or an explicit `object` type — a **typeless** schema (neither declaration: a bare `{}` or a description-only schema) is non-object and rides the synthetic `body` property exactly as an array or scalar does, whatever values it might admit at runtime.
...
The request's media type is selected from the operation's **declared** `requestBody.content`, in preference order (**OAPI-P-04**): exact `application/json`, then the lexicographically least `+json` type, then `multipart/form-data`, then `application/x-www-form-urlencoded` (fields serialize per the OAS's `encoding` rules), then `text/plain` when the body value is a string. Three refusals are pre-dispatch and loud: an operation declaring only media types outside these families (a raw binary request body, for instance — its request carriage is undefined in revision 1); a **degenerate media/schema combination** (next paragraph), whose selected media type cannot carry the declared body schema; and a selection whose condition fails (`text/plain` selected with a non-string body value). Non-JSON *response* bodies are not excluded; they decode through the text lane of [§9.4](#94-configuration-points).

**Degenerate media/schema combinations.** A selection landing on `multipart/form-data` or `application/x-www-form-urlencoded` while the declared body schema does **not** flatten under [§9.1](#91-input-mapping--the-flattened-model)'s declaration-only determination (no `properties` and no explicit `object` type: the shapes the flattened contract carries under the synthetic `body` property), or landing on `text/plain` while it **does**, has no OAS-defined wire form and is refused before dispatch rather than invented. The OAS specifies multipart and form-urlencoded serialization exclusively over object properties: its multipart considerations define the payload as an object whose properties become the parts, and its Encoding Object is keyed by property name — for a propertyless shape no rule exists to implement, and anything emitted would be implementation invention. The text lane is the inverse: a single-string wire that cannot carry a flattening schema's object contract. The refusal is decided by declarations alone, the same species as the out-of-family refusal above, and it reaches only operations declaring **no** JSON-family request media: selection prefers the JSON family, which carries any shape, so any co-declared JSON media type is selected first and the contract and the wire agree. The exclusion is a definition, not an open item: a future revision may define carriage for such a combination if the OAS or a real artifact supplies a serialization rule — an incompatible change (`openbindings.openapi@2`).
...
- **OAPI-P-03**: Input fields map per the flattened model — merge rules, cross-location same-name refusal, collision delivery, evaluation-free passthrough into an object body only where the selected request media is JSON-family, with pre-dispatch refusal of an unmatched field where no destination is derivable without invention (no body, a non-object body, or an object body whose selected media is multipart/form-urlencoded), required-declaration refusals including the missing-path-parameter case, the remaining-body rule, and synthetic `body` unwrapping — per [§9.1](#91-input-mapping--the-flattened-model).
- **OAPI-P-04**: Request media selection, its deterministic tiebreaks, its pre-dispatch refusals (the degenerate media/schema combination refusal included), multipart part encoding including the Base64 boundary encoding for binary-signaled parts, and the `Accept` membership rule follow [§9.2](#92-request-and-response-media).
