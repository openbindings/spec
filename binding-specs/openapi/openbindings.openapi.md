...
- A field matching no declared parameter or body property **passes through into the body** when the operation declares an **object** request body, and is **refused before dispatch** — loudly, never silently dropped — when it declares none. The passthrough exception exists because an object body gives an extra field a legitimate destination; where that reason evaporates, the catalog's default posture — the loud unknown-field refusal of the grpc and connect families (compare `openbindings.grpc@1`'s [§9.1](../grpc/openbindings.grpc.md#91-input-mapping), GRPC-P-03) — resumes: when the declared request body is **non-object** (the flattened contract carries the synthetic `body` property of the next rule — a string, binary, array, any non-object shape), a field matching neither a declared parameter nor `body` has no destination and is refused before dispatch, the same species of refusal as the unflattenable cross-location collision above and the missing-required-path-parameter rule below. Passthrough fields **join the body value before request-media selection** ([§9.2](#92-request-and-response-media)): they are body properties like any declared ones, riding whatever encoding the selected media type gives the body — JSON, multipart, form-urlencoded — never a side channel. No schema evaluation participates in this routing; enforcing the body schema is the server's business.
...
- **OAPI-P-03**: Input fields map per the flattened model — merge rules, cross-location same-name refusal, collision delivery, evaluation-free passthrough into object bodies joining the body value before request-media selection, pre-dispatch refusal of unmatched fields when no body — or a non-object body — is declared, required-declaration refusals including the missing-path-parameter case, the remaining-body rule, and synthetic `body` unwrapping — per [§9.1](#91-input-mapping--the-flattened-model).
