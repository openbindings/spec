...
This document also carries its own edition label: it defines **version 0.2.0** of the graph-unit format — the version each graph declares in its `openbindings.operation-graph` field ([§12](#12-operation-graph-definition)). That label is not the binding-specification revision: the proposed identifier is `openbindings.operation-graph@1`, and the graph-unit format carries its own version field, whose semantics this document defines. The graph format is pre-1.0, and minor-version updates MAY include breaking changes. Version 0.2.0 is a ground-up rewrite around the transparency principle (see [Transparency](#11-transparency-the-identity-law)); earlier draft graph formats remain available in version-control history for comparison only.
...
Two carried representations are accepted, discriminated structurally: **object** `content`, the parsed source document; and **string** `content`, its JSON source text, parsed per [RFC 8259](https://www.rfc-editor.org/rfc/rfc8259) — this family is JSON-native and defines no other grammar. Duplicate object keys are refused loudly: RFC 8259 leaves duplicate handling to the parser, so a source text carrying them has no single meaning, and a graph is not addressable from a text two parsers read differently. Revision 1 of this specification governs graph units declaring **exactly `0.2.0`**. Any other value is refused loudly per [OG-T-02](#24-conformance), including a later `0.2.x` value this document has not incorporated. Accepting a later graph-unit edition publishes a new binding-specification identifier after review, never an in-place widening.
...
The leading `#` is part of the syntax; it denotes a fragment identifier per RFC 3986 in shape only — the value is a **verbatim string**, and no URI processing applies to it. Tools that resolve `selector` MUST treat the value as a JSON Pointer, MUST NOT accept bare graph keys (e.g., `"paginateAll"`) as shorthand, and MUST NOT percent-decode it: a target has exactly one conformant spelling (RFC 6901 escaping included), matched byte-for-byte per the core specification's literal-form doctrine (core [§7](../../openbindings.md#7-reference-resolution)). A percent-encoded rendering of that spelling (`#/graphs/paginate%41ll` for `#/graphs/paginateAll`) is not the spelling and denotes nothing.

A `selector` that does not resolve, or whose resolved value is not a valid operation graph definition, is a binding error; tools acting on the binding MUST surface it.

`selector` is REQUIRED on bindings governed by this specification. To target a graph at the document root, use the JSON Pointer fragment `"#"`. This specification does not define a "whole document" interpretation for an absent `selector`.

## 8. Target and interaction

The binding target is the operation graph the binding's `selector` addresses ([§7](#7-selector)): the graph _is_ the binding, satisfying the bound operation's contract by composing other operations ([§10](#10-overview)). The interaction pattern is never declared — a graph's boundary cardinality is emergent from its contents and the bindings selected at runtime ([§10](#10-overview)) — and the observable interaction surface this specification works with is named by the interaction model below.

### Interaction model

The core specification deliberately defines no invocation API or runtime lifecycle: invocation is a processor concern ([core §1.2](../../openbindings.md#12-out-of-scope)), and an operation's interaction pattern is determined by its selected binding, not declared ([core §5.1](../../openbindings.md#51-operations)). A graph nevertheless composes the observable interactions selected bindings denote, so this specification needs names for that semantic surface. The vocabulary below is part of graph meaning; it does not prescribe a processor API or internal architecture.

- **Invocation**: one use of an operation through a selected binding, from initiation until terminal status.
- **Input side**: the sequence of values the caller **writes** into the invocation, in order, followed by a **close**. **Close responsibility**: the input side is closed by the caller, or _from below_ by the selected binding when the binding can know that no further input is meaningful (a unary binding closes after its first read). When the binding cannot know, closure rests with the caller; a caller that neither writes nor closes owns the resulting hang.
...
**Transform positions.** The graph's own `$input` context binding is defined for the graph's internal expressions ([Runtime context](#18-runtime-context), [Transforms](#transforms)) by this specification's semantics chapters — exactly as core [§10.6](../../openbindings.md#106-retired-rule-identifiers) and [OBI-T-10](../../openbindings.md#103-tool-rules) anticipate for a governing specification's own expression positions.
...
An operation graph is a small dataflow program that _is_ a binding: it implements one operation's contract by wiring other operations together. Nodes are steps; edges carry streams of events between them. The smallest graph is a bare passthrough — `input → output`, piping every caller write straight back out — but the canonical graph wraps a single operation:
...
- **Nodes are _what_; edges are _how data flows_.** A node's `type` fixes its behavior; an edge is a plain wire with no logic on it.
...
Observational equivalence is defined over the surface named in [Interaction model](#interaction-model):
...
Excluding schedules does not exclude **causal availability**: the wrapper MUST initiate the inner invocation without waiting for a caller write, and MUST consume an output that the inner invocation can produce before input. An implementation may delay delivery, but it may not introduce a dependency on future input that direct invocation does not have. Otherwise a caller that waits for an initial server output before writing would complete directly and deadlock through the wrapper.
...
- `openbindings.operation-graph` (REQUIRED, string): the version of this format the graph was authored against. Under this revision its value MUST be exactly `0.2.0` ([OG-T-02](#24-conformance)). Each graph in a document declares its own version independently.
- `description` (OPTIONAL, string): a human-readable description of what the graph does.
- `nodes` (REQUIRED): a map of named nodes. Each key is a node identifier matching `^[A-Za-z_][A-Za-z0-9_.-]*$`; each value is a [Node](#14-node-definitions). Node keys MUST be unique within the graph (for object `content` this follows from the parsed data model; for string `content` it rests on [§3](#3-accepted-source-representations)'s duplicate-key refusal).
...
- `error` (any): the failure's identifier string (see [Error identifiers](#errors)) or, for a failure originating in an inner invocation, its abstract terminal record as the JSON object `{ "code": <string> }` with `data` included exactly when it was present on the inner record. This preserves absent versus explicit-null data without carrying a language-native error object.
...
Error policy is **fatal unless explicitly handled**. For a **per-event failure** — an `each` invocation failing, a `WRITE_REJECTED` at a non-accepting `operation` node, `MAP_NOT_ARRAY`, `TRANSFORM_UNDEFINED` — the portable error event is `{ "error": <failure>, "event": <event> }`. If `onError` is set, that event is routed to the named node and the graph continues; otherwise the graph invocation terminates with the error event as its terminal data. For a **conduit terminal error**, `onError` likewise opts the error into graph handling, but the routed event is `{ "error": <failure> }` with no event member; without `onError`, the graph terminates with the inner terminal error itself, exactly as direct invocation would. The [identity law](#11-transparency-the-identity-law) requires the latter unwrapped record. This symmetric fatal default makes loss an authored choice: route `onError` to a transform for a fallback, to an operation for logging, to an `exit` node for explicit termination, or to a filtering/dead-end path for explicit disposal. A node type with no defined failure modes (`exit`, `buffer`, `combine`, and a schema-based `filter`) never fires `onError`; declaring it on such a node is valid and inert.
...
> **Note on terms.** A graph _is a binding_ in the [core sense](../../openbindings.md#53-bindings): a `bindings` entry selects it as an operation's target. When this document says "two binding **authorities**," it means _what fixes a node's behavior_ — runtime selection for an `operation` node, this specification for a built-in — not a second kind of core `bindings` entry.

**The lifting rule.** A node's behavior over a stream follows from what it is:

- _Stateless per-event built-ins_ (`transform`, `filter`, `map`) have exactly one lifting over a stream — apply per event — because for a stateless function, "one instance over the stream" and "an instance per event" are indistinguishable. No choice exists, so none is offered.
- _Stateful built-ins_ (`buffer`, `combine`) are by definition one instance per graph invocation; their cross-event state is what they are.
- _Operations_ carry session identity — state, connection, side-effect scope, lifecycle — so two distinct liftings exist: one invocation written every arriving event (`operation`, the conduit) and one invocation per arriving event (`each`). Both are cardinality-agnostic toward the inner operation: the conduit asserts nothing, and `each` fixes only the graph's own write count per session, which any caller of any operation is entitled to do. The two coincide when exactly one event arrives, and differ at zero events and at two or more (see [`operation`](#operation) and [`each`](#each)).
...
- The invocation is opened when the graph invocation opens, before the graph waits for or accepts its first caller write. Its output side is consumed immediately and independently of its input side. An output available before any input therefore flows downstream without waiting for an arriving event.
...
Opening is deliberately **not gated by whether an upstream event later reaches the node**. An `operation` node denotes a live invocation session whose input happens to be a stream; it is not a conditional task. A filter, transform, or fan-out upstream controls which writes enter that already-open session, but does not select whether the session exists. Authors who want “invoke only for events that reach this branch” use [`each`](#each) after the filtering branch; zero events then means zero invocations. This uniform session rule keeps an `operation` node's no-input and output-before-input behavior independent of incidental graph topology. The identity law requires it for the trivial wrapper, while this paragraph explicitly defines its intentional extension to every `operation` node.

Fields:

- `operation` (REQUIRED, string): the operation key to invoke.
- `timeout` (OPTIONAL, integer >= 1): total budget in milliseconds for the invocation, measured from graph-invocation start until its output stream completes; not an idle or per-event timeout. On expiry the invocation terminates with `TIMEOUT_EXCEEDED`, handled as a conduit terminal error (see **Failure** below).
...
- If the node does not set `onError`, the graph invocation terminates with the invocation's terminal error, exactly as if an `exit` node with `error: true` had been reached with the inner terminal error as the error data: previously emitted outputs are not retracted, in-flight events and inner invocations are cancelled, and the caller-facing input side is closed.

The conduit terminal-error default is forced by the [identity law](#11-transparency-the-identity-law): terminal status is on the equivalence surface, so the bare wrapper must surface the inner invocation's terminal error identically to direct invocation. Write rejections at a non-accepting node are per-event failures, not conduit terminal errors; they are routed per `onError` with the rejected event attached, or terminate the graph with that error event when unhandled.
...
`each` fixes the graph's contribution — one write per session — and asserts nothing about the operation's nature; a single write-and-close is one complete action under the [interaction model](#interaction-model).
...
- `error` (OPTIONAL, boolean, default `false`): if `false`, the event is emitted to the graph's output stream before termination (early return). If `true`, the graph terminates with an error and the event is the error data, not emitted as output.
...
Wherever a node embeds a JSON Schema (`filter.schema`, `buffer.until`, `buffer.through`), this format rules the schema's form itself ([OG-V-18](#19-validation-rules)): JSON Schema 2020-12 in object form; `$schema`, when present, equal to `https://json-schema.org/draft/2020-12/schema`; and no `$vocabulary` anywhere within it. Embedded schemas are **self-contained**: no keyword within an embedded schema references anything outside it (OG-V-18) — a graph is a movable unit, and a schema reaching outside itself would need a resolution scope no host is obliged to provide. In JSON Schema 2020-12 that property forecloses every reference keyword, not `$ref` alone: `$ref`, `$dynamicRef`, `$dynamicAnchor`, and `$anchor`-fragment references all appear nowhere within an embedded schema — the same set the core specification excludes at OBI positions ([§5.2](../../openbindings.md#52-schemas), OBI-D-05), so the alignment this section claims holds keyword-for-keyword. At evaluation, `format` is an annotation, never an assertion: an engine MUST NOT gate an event on `format` (the same rule the core specification applies at its validation boundaries — deliberately aligned, independently ruled). These are the same constraints the core specification places on schemas at OBI positions ([§5.2](../../openbindings.md#52-schemas)) — deliberately aligned, independently ruled, so a graph engine validates a graph with no core-spec lookup.
...
**Dead ends**: a node with no path to `output` is legal; its events are discarded, but its side effects occur. In particular, every `operation` node is opened even if no event reaches it; use `each` when invocation itself must be conditional on an event. A dead end is a side-effect-only path, not fire-and-forget: its inner invocations remain in flight and graph completion still waits for them unless cancellation or an `exit` tears them down.
...
When a graph-bound operation is invoked, the implementation first opens the one held invocation for **every `operation` node**, starts each declared timeout budget, and begins consuming every held invocation's output side. It completes that startup before waiting for or accepting the first caller write. Opening is not ordered by node-key or edge order, and no graph may depend on relative startup timing; the requirement is that caller input and inner output can progress independently. This is the session semantic defined under [`operation`](#operation), not an inference that every topological branch was selected: `operation` nodes are unconditional sessions, while `each` nodes are event-conditioned invocations.
...
Consequently: a graph whose input feeds only `operation` nodes closes its caller's input exactly when its contents do — the trivial unary wrapper closes input after the first write, identically to direct invocation — while a graph whose input feeds an `each` node or other built-in genuinely cannot know when input is done, so closure rests with the caller, consistent with the [interaction model](#interaction-model)'s close-responsibility rule for bindings that cannot know. Authors should note which case a graph is in; a caller that neither writes nor closes against a graph awaiting input is the same documented caller-owned hang as for any such binding.

### Graph completion

The graph invocation is complete when either (a) an `exit` node is reached, which terminates it immediately, or (b) the `input` node has completed, all events have finished flowing — reached `output`, reached a dead end, or been dropped — and no events or inner invocations are in flight. In cyclic graphs — data cycles and error routes alike — completion is judged by **quiescence**: a cycle (or an error-route feedback) is complete when every event within it has been dropped or has left it and no new event can enter. An implementation MUST complete the affected nodes at that point rather than wait on the circular edge-completion dependency the literal per-edge rule would create. The detection mechanism (reference counting, liveness tracking, drain detection) is implementation-defined — a non-portable axis as to timing only, never as to which events are delivered.
...
**Per-event failures.** When a node fails while processing an event (an `each` invocation's terminal error or `TIMEOUT_EXCEEDED`, `MAP_NOT_ARRAY`, `TRANSFORM_UNDEFINED`, a `WRITE_REJECTED` at a non-accepting `operation` node, or any other failure attributable to a single event), the failing event does not propagate along the node's normal outgoing edges. Its error event is `{ "error": <error>, "event": <eventBeingProcessed> }`, where a graph-native failure is its identifier string and an invocation failure is its `{code,data?}` record. If `onError` is set, that event is routed to the named node and other in-flight events continue normally. If `onError` is absent, the graph invocation terminates with that complete error event as its terminal error data; previously emitted caller outputs stand, while in-flight events and inner invocations are cancelled and caller input closes. Explicit disposal is expressed by routing `onError` to a path that intentionally emits nothing, never by omission.

**Conduit terminal errors.** When an `operation` node's held invocation terminates with an error, the failure belongs to the invocation as a whole. If the node sets `onError`, an error event (`{ "error": <terminalRecord> }`, with no `event` member) is routed to the named node, the node completes and is non-accepting, and the graph continues. If the node does not set `onError`, the graph invocation terminates with that error — previously emitted outputs are not retracted, in-flight events and inner invocations are cancelled, and the caller-facing input side closes — exactly as for an `exit` node with `error: true` whose error data is the inner terminal record. The graph's terminal error is the inner terminal record itself, the value direct invocation would surface, not an error-event wrapper around it; the `{ "error": ... }` shape exists only for events routed inside the graph. This fatal default is required by the [identity law](#11-transparency-the-identity-law)'s terminal-status clause; see [`operation`](#operation).
...
| `EXPRESSION_EVALUATION_FAILED` | a `transform`, `filter`, or `map` node whose expression evaluation throws            |

A failure originating inside an inner invocation (the operation itself fails) surfaces the inner abstract terminal record as the `error` value: exactly `{ "code": <string> }` or `{ "code": <string>, "data": <JSON value> }`, preserving whether `data` was absent or explicitly null. These records are not drawn from the identifier table below. An expression **evaluation failure** (a throw — a JSONata `$error()`, a type error, an unknown function — as distinct from an undefined result) is a per-event failure whose portable `error` value is `EXPRESSION_EVALUATION_FAILED`; processor-specific diagnostic prose may be surfaced out of band but never replaces that identifier in the graph event. It MUST NOT be `TRANSFORM_UNDEFINED`, which is reserved for expressions that evaluate to no result — the classes are distinct. Which evaluations throw and which yield undefined follows the JSONata reference implementation, per this document's [Transforms](#transforms) deference. Because inner failures are objects while graph-native identifiers are strings, the two namespaces do not collide. Future versions of this format may add identifiers; graphs that route on `error` values should treat unrecognized identifiers as ordinary failure values, not errors in themselves.
...
- A node that merges several events into one output — `buffer` on flush, `combine` on emit, and an `operation` node — gives the merged output, for each `each` node, the **maximum** of that count among the contributing events. For `buffer` and `combine`, contributors are the events in that flush/emission. For an opaque `operation` conduit, contributors are conservatively every input event whose write the held invocation accepted **before that output was emitted**; an output available on open has no contributors. A later write never retroactively changes an already-emitted output's lineage. A merge never lowers a count, so merges cannot be used to escape a bound.
- An `onError` route carries the failing event's lineage; for a conduit terminal error, which has no single failing event, the error event carries the merged lineage of every input event whose write was accepted before the terminal error occurred, per the merge rule above. `onError` references count as edges for cycle detection (OG-V-09), so error loops are bounded identically to data loops.
...
- **Eventual completion.** In the absence of cancellation, inner-operation/environment failure, or a declared wall-clock timeout expiring, a graph that these semantics say completes will complete under every conforming implementation; completion is determined by the data, not by scheduling.
...
- **Whether and when a declared wall-clock timeout expires**, because transport latency, processor scheduling, backpressure, and host load are environmental inputs. A graph that needs outcome portability across environments must not make its successful result depend on completing within such a budget; timeout remains a safety policy with a portable failure identifier, not a deterministic logical clock.
...
A graph's _topology_ is easily mistaken for an _ordering_; they are not the same. The clearest trap is the `map → each → buffer` shape of [Example 4](#example-4-map-and-collect): `map` emits the ids in array order, but `each` opens one invocation per id and those invocations run concurrently. Because **interleaving across concurrent `each` invocations' outputs** is implementation-defined, the order in which results arrive at the `buffer`, and so the element order of the single array it emits, is not guaranteed. Example 4 guarantees the _multiset_ of results, not the _sequence_: a consumer reading `[Alice, Bob, Carol]` positionally relies on one implementation's scheduling, not on this format. The "single linear path" of edges is a red herring — the `each` node is a concurrency point sitting on it.

A second instance is concurrent paths racing a shared sink. A `map` or fan-out feeding both an `output` path and an `exit` does not guarantee _how many_ events reach `output` before the `exit` fires; `exit` terminates "when an event reaches this node," and which concurrently-in-flight events were already emitted is a timing property the [identity law](#11-transparency-the-identity-law)'s equivalence surface excludes.
...
- `$` — the current event.
- `$input` — the **root input event of the current event's lineage**: the caller-written value this event descends from. Immutable. For an event descending from a merge (`buffer` flush, `combine` emission, or an `operation` conduit written from multiple lineages), `$input` is defined if and only if all contributors defined by [event lineage](#maxiterations-and-event-lineage) share one root; otherwise `$input` is unbound (undefined) during evaluation. For a conduit output, the contributor cutoff is the instant that output is emitted: accepted earlier writes contribute, later writes never do. An `operation` output emitted before that invocation receives any input therefore has no root and an undefined `$input`. The expression's overall result then governs, per the undefined-result rule under [Transforms](#transforms): an expression whose result is undefined fails the node with `TRANSFORM_UNDEFINED`, while an expression that handles the undefined `$input` and yields a defined result proceeds normally (a `filter` expression yielding a defined falsy result drops the event as usual). In a graph invoked with a single write — the common case — events descending from that write share its root and `$input` behaves as a constant.
...
The following rules apply to each operation graph definition (the value at which a binding's `selector` resolves). Enforcement is a tool obligation: a tool acting on an operation-graph binding MUST validate the graph against these rules before acting on it ([OG-T-01](#24-conformance)). Each rule carries a stable identifier (`OG-V-##`) so validators, fixtures, and errata can cite it unambiguously; identifiers are never reused, never renumbered. The per-node field-presence requirements marked (REQUIRED) in the node definitions above are equally normative and are encoded by this format's JSON Schema; a validator applies both the enumerated rules and node well-formedness (required fields present, defined fields well-typed).

This format's `openbindings.operation-graph.schema.json` expresses the structural portion for validator tooling. It is a **derived artifact, not a second source of truth: where prose and schema conflict, the prose governs** (the same posture core [§10](../../openbindings.md#10-conformance) takes toward `openbindings.schema.json`). Graph objects are closed except for the explicit `x-` extension namespace: an unknown non-extension field violates OG-V-19, while an `x-` field follows [§20](#20-extensions). Exact graph-unit versioning supplies forward evolution; silently ignoring an executable-field typo does not.

- **OG-V-01**: The graph MUST declare an `openbindings.operation-graph` string field matching the SemVer 2.0.0 pattern. Whether this specification accepts that well-formed version is the distinct processor gate OG-T-02.
...
- **OG-V-18**: Every schema embedded in a graph definition (`filter.schema`, `buffer.until`, `buffer.through`) is a JSON Schema 2020-12 object; its `$schema`, when present, equals `https://json-schema.org/draft/2020-12/schema`; and it is **self-contained** — no keyword references anything outside it, which forecloses `$vocabulary` and every reference keyword the dialect defines: `$ref`, `$dynamicRef`, `$dynamicAnchor`, and `$anchor`-fragment references all appear nowhere within it (see [Embedded schemas](#embedded-schemas); the same set core [OBI-D-05](../../openbindings.md#102-document-rules) excludes at OBI positions).
- **OG-V-19**: At the graph, node, and edge object levels, every field is either defined for that exact object/type or begins with `x-`. An unknown non-`x-` field, including a field defined for a different node type, is a validation failure rather than ignored executable text.
...
- Graph definitions MAY include extension fields whose keys begin with `x-` at any graph-controlled object location (the graph itself, nodes, edges).
...
- A field that is neither defined for that exact graph/node/edge object nor `x-`-prefixed is a validation failure under OG-V-19. This intentionally differs from the core's open OBI object model: graph definitions are executable artifacts with their own exact version gate and extension namespace. A mistyped `timeot` must not silently remove a safety budget, and `maxIterations` on an `operation` node must not look effective when it belongs only to `each`.
...
**Execution (error path)**: an item's fetch times out; the error event `{ "error": { "code": "TIMEOUT_EXCEEDED" }, "event": { "id": "item-1" } }` routes to `fallback`, which reads `event.id` and the complete `error` record (distinct from `$input`, the lineage root) and emits `{ "id": "item-1", "name": "unknown", "error": { "code": "TIMEOUT_EXCEEDED" } }` to `out`. Other items continue unaffected.

### Example 8: Fatal per-event error with `exit`

All unhandled node failures are fatal by default (see [Errors](#errors)). This example makes the policy explicit by routing an `each` failure to an `exit` node, which also gives the graph an obvious place to add centralized error transformation later:
...
**Execution (error)**: one item's invocation times out; the error event `{ "error": { "code": "TIMEOUT_EXCEEDED" }, "event": { ... } }` routes to `die`; the graph terminates with the error event as its error data. Previously emitted outputs are not retracted; in-flight invocations for other items are cancelled; the stream closes as failed. Without `onError`, the graph would terminate with the same error event under the fatal default; the explicit route documents where this graph applies its policy.
...
- **OG-D-03**: `selector` is present and is an [RFC 6901](https://www.rfc-editor.org/rfc/rfc6901) JSON Pointer fragment resolving to a graph definition (`"#"` addressing a root-level graph), in the fragment's one verbatim spelling — never percent-decoded — per [§7](#7-selector).
- **OG-T-01** (acting on an operation-graph binding: validating, generating code, invoking): MUST validate the target graph definition against the [validation rules](#19-validation-rules) (OG-V-01 through OG-V-19) and MUST fail the binding rather than act on a graph that violates them.
- **OG-T-02** (all processors): MUST accept this revision's graph-unit version only when `openbindings.operation-graph` is exactly the string `0.2.0`, and MUST report any other version as a version refusal distinct from graph non-conformance ([OG-T-01](#24-conformance)). SemVer precedence, build metadata equivalence, and inferred patch compatibility do not widen this exact incorporated set. Accepting a later graph-unit value publishes a new binding-specification identifier.
- **OG-T-03** (executing graphs): MUST evaluate node expressions in the pinned language of [Transforms](#transforms) — JSONata 2.1, jsonata-js 2.1.1 tiebreak — whose evaluation commitments deliberately align with the core specification's binding transforms ([§10.6](../../openbindings.md#106-retired-rule-identifiers)).
...
- **[RFC 6901]** "JavaScript Object Notation (JSON) Pointer." <https://www.rfc-editor.org/rfc/rfc6901>. Incorporated authority for `selector` addressing ([§7](#7-selector)).
