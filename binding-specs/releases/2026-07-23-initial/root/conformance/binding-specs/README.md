...
fixtures live in six family directories; processor scenarios live in
...
(`invalid` source units do not count as upstream-valid). The initial six
scenarios exercise all six published families and mix faithful targets with
...
coverage for all six families. Synthesis scenario files are likewise checked
for all six families, including target/disposition consistency. It does not
