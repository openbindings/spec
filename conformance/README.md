...
**Document validity coverage is complete for OBI-D-01 through OBI-D-12 and OBI-D-16 (same-document schema $ref integrity); OBI-D-13 (binding sufficiency) is partial by its per-family nature, and the 0.2.0 rewrite's new rules (OBI-D-17, OBI-D-18) are deferred to the scheduled corpus pass (below). OBI-T (tool behavior) coverage spans the parse/load-shaped rules (OBI-T-01, OBI-T-03, OBI-T-04); the remaining tool rules are deferred pending richer fixture formats. See `manifest.json` for current counts.**
...
| OBI-D-17 | **Deferred.** Schema well-formedness at every schema position (boolean or object form, meta-schema-valid). Fixture file pending the 0.2.0 corpus pass. |
| OBI-D-18 | **Deferred.** Transform parse-validity (every transform expression parses as JSONata 2.1). Needs an evaluator-backed harness rather than schema validation alone. Fixture file pending the 0.2.0 corpus pass. |
...
| OBI-T-16 | **Deferred.** Claim-triggered validation semantics (complete statically reachable schema graph, `format` as annotation, per value; unresolvable graph reported distinctly). Fixtures need value + expected verdict shape. Pending the 0.2.0 corpus pass. |
| OBI-T-17 | **Deferred.** Verification conclusions (conformant / non-conformant / undetermined under partial verification). Fixtures need a conclusion-shaped outcome, not `valid: true|false`. Pending the 0.2.0 corpus pass. |
| OBI-T-10 | **Deferred.** Transform evaluation per JSONata 2.1. Fixtures need richer shape: transform input + expected transform output. Pending fixture format design. |
