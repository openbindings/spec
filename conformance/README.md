...
| OBI-D-18 | Complete. Transform parse-validity at named and inline positions under the pinned language with §5.5 clause 7's numeric-range exception. Expressions that fail evaluation can be positives; a range-limited literal does not excuse other syntax errors. A validator without the required parse capability leaves the rule unverified rather than rejecting a document for numeric capacity. |
...
| OBI-T-06 | **Deferred.** Applies to tools that resolve `selector` values per binding-format conventions. Conformance is per-format and depends on each format community's `selector` syntax; a portable corpus would need a per-format fixture set. |
| OBI-T-16 | Complete through portable tool scenarios, independently executed by the Go and TypeScript adapters. Cases distinguish success, instance mismatch, and graph unavailability; require an unavailable reachable alternative to prevent success; exclude an unreferenced `$defs` entry from the reachable graph; treat `format` as annotation; apply schemas per value; preserve the OBI document as the same-document reference root without treating unknown root fields as schema assertions; and recognize an absolute `$ref` satisfied by an embedded `$id`. The scenario format defines semantic inputs/outcomes, not an SDK API or error serialization. |
...
| OBI-T-10 | **Deferred in the core tool-scenario format; evaluation witnesses live in [`transforms/`](transforms/README.md).** `agree/` records pinned-reference regression outcomes; `numerical/` adds rule-labelled, raw-JSON scenarios for §5.5 clause 7, including model-sensitive outcomes and unchanged-language controls. The repository verifies their structure and bounded contract witnesses; evaluator execution and full-language qualification remain separate. No finite expression set establishes the absence of every host-environment extension. Parse-validity of document transforms is covered by OBI-D-18. |
...
Three subcorpora live alongside the core corpus, each with its own fixture format. The core tooling below scans `document/`, `tool/` and `scenarios/`; the subcorpora have separate repository verifiers and execution harnesses:
...
| [`transforms/`](transforms/README.md) | Pinned-reference regression outcomes, recorded implementation divergences, and model-sensitive numerical scenarios under §5.5 | `node scripts/verify-transform-corpus.mjs` and `node scripts/verify-transform-numerical.mjs` (integrity and bounded contract witnesses, not evaluator execution); implementation harnesses located via `OB_SPEC_CORPUS` |
...
conformance/
  ...
  transforms/          (transform subcorpus; own README, verifiers and evaluator harnesses)
```
