...
| OBI-D-18 | Complete. Transform syntax at named and inline positions under the documented JSONata 2.1 language. Positives include expressions whose evaluation can fail. A validator without the required parse capability leaves the rule inconclusive. |
...
| OBI-T-10 | **Deferred in the core tool-scenario format; evaluation witnesses live in [`transforms/`](transforms/README.md).** `language/` records authority-linked semantics and OpenBindings result/environment controls. `agree/` and `known-divergence/` retain historical implementation observations, not a blanket conformance oracle. Repository checks validate fixtures; evaluator and closed-environment qualification remain separate. |
...
| [`transforms/`](transforms/README.md) | Authority-linked language semantics and Core boundary controls, alongside historical implementation observations | `node scripts/verify-transform-corpus.mjs` and `node scripts/verify-transform-authority.mjs` (fixture integrity, not evaluator execution); implementation harnesses located via `OB_SPEC_CORPUS` |
