...
| OBI-T-06 | **Deferred.** Applies to tools that resolve or act on `selector` values. What a `selector` means is each binding specification's to define, so conformance is per binding specification; a portable corpus would need a fixture set per specification. |
