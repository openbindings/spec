...
| Failure payloads can contain application-authored data alongside native protocol evidence. | Do the binding rules identify the application value precisely while keeping protocol containers below the bridge? | MCP `isError` results complete unsuccessfully; declared `structuredContent`, when present, crosses exactly as application-authored `data`, while the complete MCP result remains native evidence. |
