...
OBI sources declare **exact** format versions (e.g., `openapi@3.1`). These are precise identifiers that describe a specific binding artifact. Invokers declare **format ranges** (e.g., `openapi@^3.0.0`) that describe the set of source versions they can handle. The SDK's format matching logic checks whether an OBI source's exact token falls within an invoker's declared range. The core spec defines the `format` field on sources (see `openbindings.md` section 6.4) but leaves format-token equivalence and compatibility to each format's community. The conventions below describe how the official tooling handles common formats.

---

## OpenAPI

**Invoker format range**: `openapi@^3.0.0` (handles any OpenAPI 3.x source)
...
**Credentials**: Driven by the spec's `securitySchemes`. The invoker reads the OpenAPI security configuration to determine where credentials go (header, query, cookie). Falls back to bearer, then basic, then apiKey when no schemes are defined.
...
**Invoker format range**: `asyncapi@^3.0.0` (handles any AsyncAPI 3.x source)
...
**Protocol dispatch**: The invoker determines the transport from the server protocol and operation action:
...
**Invoker format range**: `grpc` (versionless)
...
**Connect compatibility**: [Connect](https://connectrpc.com) servers expose the gRPC protocol alongside the Connect protocol by default. The gRPC invoker can discover and invoke against them via gRPC. The resulting OBI reflects the gRPC access path, not the Connect protocol. Connect-native access (HTTP/1.1, JSON payloads) uses the dedicated `connect` format — see [Connect (Buf)](#connect-buf) below.
...
**Invoker format range**: `connect` (versionless)

**Ref format**: Same as gRPC: `{package.Service}/{Method}`. Both formats share protobuf service definitions.

**Source**: `location` is the Connect server base URL. The invoker constructs `{location}/{service}/{method}` for each RPC. `content` accepts inline protobuf definitions for proto-aware marshaling.
...
**Credentials**: Applied as HTTP headers: bearer, apiKey, basic. Same chain as other HTTP-based invokers.
...
**Invoker format range**: `mcp@2025-11-25` (exact, date-versioned)
...
**Entity semantics**: Tools map to traditional API operations. Resources are read-only data access (URI stored as a `const` input property). Prompts return LLM message sequences. Each invocation creates a fresh MCP session.
...
**Invoker format range**: `graphql` (versionless)
...
**Query construction**: GraphQL requires a full query string with a selection set. The invoker checks the operation's input schema for a `_query` property with a `const` value. If present, that query is sent directly with the caller's input as variables. If absent, the invoker falls back to building a query from introspection with an auto-generated selection set (depth-limited to 3 levels, cycle-safe).

The interface creator generates the `_query` const at OBI creation time from the introspected schema, so any generated OBI will have precise queries. The introspection fallback is for direct invoker use without an OBI.
...
**Invoker format range**: `usage@^2.0.0` (handles any Usage 2.x source)
...
**Credentials**: Local CLI invocation, not network services. Configuration passed via `InvocationOptions.Environment` as environment variables.
...
**Security metadata**: Not applicable (local invocation).
...
- **Server-streaming** (one request, many responses): the invoker emits each response as a separate stream event.
- **Server-Sent Events (SSE)**: the invoker parses the `text/event-stream` body line by line and emits each event as a stream event.
- **Subscription with single input**: the caller provides parameters once; the invoker opens a long-lived connection (e.g., WebSocket, AsyncAPI subscription, GraphQL subscription, MCP resource subscription) and forwards each server-pushed message as a stream event.
- **Progress notifications during a long-running call**: when the underlying protocol surfaces intermediate progress events during a single RPC (e.g., MCP `notifications/progress`), the invoker MAY surface them as intermediate stream events, with the final result as the last event.

---

## Field naming across protocol locations (non-normative)

Many binding formats present parameters from multiple protocol locations (path, query, headers, body) as a single object-shaped view for schema comparison and invocation tooling. In that flattened representation, each field name maps to at most one value. If a binding source reuses the same name in more than one protocol location (for example, `id` as both a path parameter and a body field), the flattened view cannot distinguish which value belongs where.

Object-shaped operation schemas have a related constraint: within a JSON object, property names are unique keys. OpenBindings works best when a binding source can be represented with unique field names across the effective input/output surface. When collisions are unavoidable, binding-format-specific or invoker-specific handling is required.

---

## Conventions for new formats

If you're building an invoker for a new binding format, you should define and document:
