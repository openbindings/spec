...
- **Untyped runtime** — the SDK's `OperationInvoker`. Works with any OBI. Operations are strings, input/output are untyped. The dispatcher that routes operations to binding invokers.
...
- `OperationInvoker`: the runtime dispatcher. Routes operation invocation to binding invokers by reading the OBI's bindings to find which invoker handles each operation. Applies transforms if configured.
...
Each binding invoker advertises the format tokens it handles (e.g., `openapi@3.0`, `asyncapi@3.0`). The dispatcher matches operation bindings to invokers by format.
