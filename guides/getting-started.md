...
The OBI doesn't implement protocol logic. It just tells binding invokers where to look. See [Consuming an OpenBindings Interface](consuming-an-interface.md) for the consumer side, or the [role pages](../interfaces/) for what a binding invoker (and the other roles) actually do.
