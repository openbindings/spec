# OpenBindings 0.2 Core Specification Content Analysis (archived)

## Status and scope

This archived document is a non-normative analysis that informed the
OpenBindings 0.2 working draft. It is retained as design history, is not part
of the specification, and does not define current work or conformance. Its
purpose was to separate the specification's content by its distance from the
central OpenBindings premise, then evaluate whether each major commitment was
necessary, well placed, proportionate, and sufficiently justified.
