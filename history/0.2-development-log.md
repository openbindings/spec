# OpenBindings 0.2 development log (archived)

This is the chronological, session-level record that accumulated while the
0.2 working draft was designed. It is preserved for design archaeology, but
it is not the release changelog, a migration guide, or a current issue list.
See [`../CHANGELOG.md`](../CHANGELOG.md) for the concise release delta and
[`../MIGRATING-0.1-TO-0.2.md`](../MIGRATING-0.1-TO-0.2.md) for migration
instructions.
...
- **Publication-lifecycle reset** (2026-08-11; recorded 2026-08-13). Between
  2026-07-23 and 2026-08-10 the binding-spec publication lifecycle was
  exercised against development drafts of the family candidates: seventeen
  revision entries across the eight families (openapi through `@7`; asyncapi,
  graphql, and mcp through `@2`; connect, grpc, usage, and operation-graph at
  `@1`), with digest-chained bundles under `binding-specs/releases/` and a
  populated `publications.json`. On 2026-08-11 all of it was deleted and every
  family was reset to a single mutable `@1` candidate.

  **Ruling (2026-08-13): the project does not regard those artifacts as
  publications.** None was referenced by any tagged release (`v0.1.0` carries
  no `binding-specs/`), none was announced, and no consumer existed; they were
  the publication machinery exercised as part of development, and using the
  immutable identifier space as a working buffer was itself the defect being
  corrected (as the 0.2-readiness review series had flagged). Because no
  identifier is regarded as minted, the `@1` candidate spellings were
  retained: today's `@1` candidates are those spellings' first real
  proposals, and July-era drafts bearing the same spellings are superseded
  working texts — distinguishable by this record, not by the identifier.

  The deleted artifacts remain citable: their complete inventory and digests
  are preserved in `publications.json` under `developmentExercises`, and the
  full pre-reset tree is at spec commit `9424bdc`. The reset commit
  (`d04b047`) landed without any record of the event and rewrote one prior
  entry of this log in place; that entry is restored below with a dated
  correction appended, because this log is append-only. Alongside this
  record, the publication verifier gained a committed floor plus tombstone
  rules so a future removal of a *published* entry cannot pass verification
  silently, and the publish script now refuses to mint without a named
  adjudication record.
...
- **GraphQL revision-1 promotion** (2026-07-23).
  ...
  *Correction (2026-08-13).* The text above is this entry's original
  2026-07-23 wording, restored verbatim. "Published binding specification
  with an immutable defining bundle" described the output of the publication
  machinery as it was being exercised during development; under the
  publication-lifecycle reset ruling recorded above, those artifacts were
  withdrawn on 2026-08-11 and are not regarded as publications, and no
  identifier is regarded as minted. The candidate's envelope and subscription
  behavior were later superseded by the current root-application-value
  semantics; its evidence was development evidence. The 2026-08-11 reset
  commit had rewritten this entry in place to assert that conclusion
  retroactively; the original wording is restored and this dated correction
  appended instead, because a chronological log records what was believed
  when, not what later proved true.
...
- **`format` renamed to `bindingSpec`.** A source names its governing **binding specification** by an exact, opaque identifier (`openbindings.openapi@1`), never dereferenced, with no version algebra; the binding specification defines the source's representations, addresses, `ref` syntax, composition, and boundary correspondence (§6, OBI-B-01..03). Mutable first-`@1` candidates for the project families are developed in `binding-specs/`; none has been published.
