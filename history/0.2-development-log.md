...
- **GraphQL first-candidate qualification pass** (2026-07-23; superseded).
  This development pass briefly described `openbindings.graphql@1` as
  published. That claim was incorrect: no binding specification has ever been
  released by the project, and no identifier was minted. The work qualified
  an earlier mutable draft of the first `@1` candidate and replaced the
  pre-`bindingSpec` `graphql` token in development implementations. Later
  fidelity review superseded its envelope and subscription behavior with the
  current root-application-value candidate semantics. The retained portable
  evidence was development evidence, not publication evidence.
