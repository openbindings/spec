...
2. A schema `$ref` (whether on an operation `input`/`output`, in the `schemas` map, or nested as a subschema within any of these): a same-document fragment in JSON Pointer form (a bare `#` or `#` followed by an [RFC 6901](https://www.rfc-editor.org/rfc/rfc6901) pointer) or an absolute URI. Plain-name (`$anchor`) fragments are not used as same-document references at OBI positions: the `schemas` map is the document's named-schema mechanism (`#/schemas/<name>`). A fragment carried on an absolute URI belongs to the external resource it addresses, and a `$ref` within a schema that declares its own `$id` resolves against that resource's base per JSON Schema — both are out of scope for this clause, the latter exactly as for [OBI-D-16](#142-document-rules). A schema `$id`, when present, is absolute.
...
- **OBI-D-16**: Has every schema `$ref` that is a same-document fragment resolving, from the OBI document root per [§10. Reference resolution](#10-reference-resolution), to a location that exists within the document. A `$ref` within a schema that declares its own `$id` resolves against that resource's base and is out of this rule's scope; an absolute-URI `$ref` is out of scope too unless it matches an embedded schema's `$id` — external resolution is an evaluation-time concern, and "external" means not resolvable within the document per [§10](#10-reference-resolution). Together with [OBI-D-08](#142-document-rules), [OBI-D-09](#142-document-rules), and [OBI-D-10](#142-document-rules), this completes one posture for the document's internal references: a same-document reference is document integrity, offline-decidable by any validator; only external resolution is deferred to evaluation.
