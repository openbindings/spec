...
2. **Evaluation.** Evaluation MUST follow the pinned language's syntax and semantics. Conformance is to the documentation, not to any implementation of it: a tool need not use, port, or reproduce the reference implementation, or any other, to conform. The semantics apply to values of the JSON data model ([§5](#5-document-model)): a tool evaluates natively over its own representation of those values and need not serialize them to JSON text. Where a host cannot compute what an expression means for the values it is given, such as a number beyond the host's range or precision or a regular-expression feature its engine lacks, the evaluation is a transform-evaluation failure (clause 4); a tool never returns a different successful value in its place.
