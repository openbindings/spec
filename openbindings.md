...
- Clients SHOULD send `Accept: application/vnd.openbindings+json, application/json` to signal media-type preference. Servers MAY content-negotiate on this header but MUST NOT refuse a request that lacks it when they otherwise serve an OBI at this path. Negotiation may vary the representation only when the request's `Accept` explicitly prefers a non-OBI media type and admits no OBI-acceptable one (`application/vnd.openbindings+json`, `application/json`, or a matching wildcard such as `*/*` or `application/*`); a request with no `Accept` header, or whose `Accept` admits an OBI type, receives the OBI body. This is ordinary proactive content negotiation ([RFC 9110](https://www.rfc-editor.org/rfc/rfc9110)) with the OBI as the default representation: a publisher MAY, for example, serve a human-readable HTML page to a browser that asks for `text/html`, without affecting any client that follows the rules above.
...
- **OBI-D-16**: Has every schema `$ref` that is a same-document fragment resolving, from the OBI document root per [§10. Reference resolution](#10-reference-resolution), to a location that exists within the document. A `$ref` within a schema that declares its own `$id` resolves against that resource's base and is out of this rule's scope; absolute-URI `$ref`s are external and their resolution is an evaluation-time concern (§10). Together with [OBI-D-08](#142-document-rules), [OBI-D-09](#142-document-rules), and [OBI-D-10](#142-document-rules), this completes one posture for the document's internal references: a same-document reference is document integrity, offline-decidable by any validator; only external resolution is deferred to evaluation.
...
- **OBI-T-07** (applies when invoking): A tool MUST validate caller-provided input against the operation's `input` schema when that schema is specified. Validation is against the fully resolved schema: a tool that cannot fully resolve it (an external `$ref` it declines or fails to fetch) MUST treat the invocation as failed — an invocation error naming the unresolvable reference — rather than validate partially. If the binding declares an `inputTransform`, validation MUST occur before transform application. A validation failure is an invocation error for that operation; it does not render the document non-conformant.
