...
- **OBI-T-09** (applies when selecting among multiple bindings for the same operation): Selection applies only among the *candidate* bindings the tool can act on. A tool need not include in the candidate set a binding whose source format it does not support, whose `ref` it cannot resolve, or that it declines for local policy. Among the candidates, a tool MUST prefer non-deprecated bindings over deprecated ones. A deprecated binding with any `priority` value is ranked after every non-deprecated binding. Within each tier, tools SHOULD prefer lower effective `priority` values, where a binding's effective `priority` is its own `priority` when present and otherwise its source's `priority` (binding-level `priority` overrides the source default rather than combining with it, per [§6.3. Bindings](#63-bindings)); bindings that share the lowest effective `priority`, or carry none, are ordered tool-defined per [§6.3](#63-bindings). Tools MAY apply additional selection tactics (latency, cost, protocol preference) after the tier rule has been applied.
