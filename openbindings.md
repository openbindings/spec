...
A binding specification MAY define a carried symbol itself as target identity — for example, a DNS name or a named runtime service-binding slot. Runtime naming infrastructure may resolve that already-identified symbol to a live handle, and may rebind it over time, without violating sufficiency. That resolution is reachability only when it does not supply a missing vendor, artifact, catalog, or service identifier; select a target by criteria absent from the binding; or reinterpret the symbol. If external state determines what target the document meant rather than how to reach the target it names, the binding is not sufficient. Verifying this distinction takes binding-specification knowledge; a validator without it leaves the rule unverified rather than failing the document ([§10.5](#105-verification-conclusions)).
...
1. **Pinned language.** OpenBindings 0.2 uses the **JSONata 2.1** language as documented in the official repository's [`version-2.1.0` documentation snapshot](https://github.com/jsonata-js/jsonata/tree/5d1473277e0022d8580e00f891b12080eb3edd74/website/versioned_docs/version-2.1.0) at source commit `5d1473277e0022d8580e00f891b12080eb3edd74`. Features introduced after JSONata 2.1 are not part of the 0.2 transform language; a later OpenBindings version may adopt a later target. The language version is bound to the OpenBindings specification version; documents carry no per-document language field. The rendered documentation is informative if it ever differs from this snapshot.
2. **Behavioral tiebreak.** Where the 2.1 documentation leaves observable behavior ambiguous, or where the initial 2.1.0 implementation was corrected within its maintenance line, the behavior of the JavaScript reference implementation at **2.1.1**, official tag commit [`5d1473277e0022d8580e00f891b12080eb3edd74`](https://github.com/jsonata-js/jsonata/tree/5d1473277e0022d8580e00f891b12080eb3edd74), governs normatively, subject to clause 7's numerical latitude. The OpenBindings project MAY publish errata against this pin; each erratum is identified and dated, and takes normative effect only when adopted by a subsequent specification release — the evaluation contract a released specification version denotes never changes after release, including the numerical latitude it permits. The expected correction vehicle is a patch release adopting the erratum; documents opt in by declaring the patched version. Any implementation may provide its own evaluator; its 0.2 transform capability implements this pinned behavior subject to clause 7.
...
6. **Determinism.** The standard library's nondeterministic functions (`$now()`, `$random()`, `$millis()`) remain available; their use is not a conformance defect. The portability guarantee is correctness of evaluation, not byte-equivalence of outputs. Clause 7 also permits precision-dependent differences in values and evaluation paths: conformance alone does not establish identical numerical results across implementations using different permitted numeric rules.
7. **Numerical latitude.** The reference's numeric precision and range are not mandatory upper limits. An evaluator MAY retain the pinned numerical behavior or use exact values and computation, or higher-capacity numerical models, under the following conditions. This permission does not require exact arithmetic or exact preservation of all JSON numbers; it is an exception within the transform contract, not a general JSON-processing requirement.

   **Eligible capacity and rounding.** Each finite-precision context replacing reference numerical behavior MUST use a radix `b` and significant-digit precision `p` satisfying `b^(p-1) >= 2^52`, where `b` and `p` are integers and `b >= 2`. For variable precision, the minimum precision used MUST satisfy this condition. This requires at least 53 binary or 17 decimal significant digits; it bounds normal-range relative spacing, not the error of every result. The supported finite magnitudes MUST extend at least from `2^-1074` through `(2 - 2^-52) * 2^1023`, inclusive at both ends in magnitude capacity, without requiring a different radix to represent each binary64 value exactly. Gradual-underflow spacing MUST be no greater than `2^-1074`. Implicit numerical rounding MUST select a nearest representable value, with exact ties resolved to an even significand integer. Exact steps need no rounding context. Overflow remaining under the stated numerical rules is an evaluation failure, not permission to saturate to a finite maximum. These capacity conditions do not promise unbounded resources; a resource failure is not successful numerical support.

   **Permitted departures.** A departure MUST result from applying those representation and rounding rules to the pinned calculation, or from computing the operation's specified mathematical meaning with those rules. The mathematical operation, type requirements, explicitly specified rounding, formatting, sequence and array rules, and clauses 3–5 remain in force. Greater capacity does not license an unrelated algorithm or a new display-precision policy. Consequences of a permitted numerical difference — including comparisons, selections, control flow, and avoidance of a numeric-range error — are also permitted. Capacity is not a guarantee that every result is closer to the exact mathematical answer than the reference result; choosing opportunistically between outcomes is not a numerical model.

   **Specified behavior and coherent values.** An evaluator exercising this permission MUST specify its numeric admission and construction rules (including literals and conversions), arithmetic and rounding rules, the value assigned to a computed result, comparison rules, and range limits sufficiently to determine its permitted numerical outcomes. The rules MUST also specify transitions between retained reference calculations and extended calculations. Merely naming a library or claiming higher precision is insufficient. Evaluation MUST follow those rules consistently. Once assigned, a number has one value: numeric comparison MUST NOT reinterpret or re-round it according to its origin or storage representation. For finite numbers, equality and ordering MUST compare their assigned mathematical values, using the same relation wherever the language compares numbers, including within composite values. A rounded computed result is compared as that assigned result, not as a hidden pre-rounding value. These provisions do not change non-numeric comparison rules. An extended numeric carrier MUST remain a number in the language, not become an observable object or string because of its host representation.

   **Syntax versus range.** A numeric literal's spelling and syntactic position remain governed by the pinned grammar; inability to represent its value as a finite binary64 number is not a syntax defect. A tool MUST NOT report document non-conformance solely for that reason. An evaluator unable to support its value MAY report a transform-evaluation failure under clause 4. All other lexical and static parse rules remain pinned, including when the standard `$eval` function parses an expression during evaluation. For example, `1e400` is syntactically valid but can fail evaluation; `1e400 +` is still syntactically invalid. Ignoring a numeric-range error without checking the rest of the expression does not verify syntax.
...
| Verify document conformance | OBI-D-02 through OBI-D-13 and OBI-D-16 through OBI-D-19, [§10.5](#105-verification-conclusions) | Record unsupported checks as unverified; use OBI-T-17 only when reporting an overall conclusion. |
...
Invocation is intentionally absent as a universal checkpoint: selecting or invoking a binding does not itself imply schema validation, transform evaluation, automatic selection, retry policy, or full document verification. Those obligations attach only when the corresponding capability is exercised or claim is made.
...
- **OBI-D-13**: Has every binding identifiable, under its source's governing binding specification, from the binding and its referenced source alone, with no dependency on external registries, vendor catalogs, or environment configuration to supply missing target identity. Binding-specification-defined symbolic identity may use runtime naming infrastructure only to resolve the carried symbol to a live handle, as distinguished in [§5.4](#54-sources). Verification is per-binding-specification knowledge; unverified is not non-conformant.
...
- **OBI-D-18**: Has every transform expression in the document — every value in the `transforms` map and every inline string value of `bindings[*].inputTransform` and `bindings[*].outputTransform` — parsing as a syntactically valid expression of the pinned transform language ([§5.5](#55-transforms): JSONata 2.1, with jsonata-js 2.1.1's parse acceptance as the normative tiebreak, subject to release-adopted errata and clause 7's numeric-range exception). The rule is syntactic membership only: it does not establish that evaluation succeeds, that referenced data exists, or that dynamic errors will not occur — those remain evaluation outcomes under [§5.5](#55-transforms). Verification note (informative): checking requires a parser for the pinned language with that exception; a validator without one leaves the rule unverified.
...
- **[JSONata 2.1]** JSONata Project, "JSONata Documentation," [`version-2.1.0` source snapshot](https://github.com/jsonata-js/jsonata/tree/5d1473277e0022d8580e00f891b12080eb3edd74/website/versioned_docs/version-2.1.0) at commit `5d1473277e0022d8580e00f891b12080eb3edd74`; the [rendered documentation](https://docs.jsonata.org/) is informative if it differs.
- **[jsonata-js 2.1.1]** JSONata Project, `jsonata` (npm), version 2.1.1, official tag commit [`5d1473277e0022d8580e00f891b12080eb3edd74`](https://github.com/jsonata-js/jsonata/tree/5d1473277e0022d8580e00f891b12080eb3edd74). Normative behavioral tiebreak for the pinned transform language per [§5.5](#55-transforms); errata against the pin are adopted only by subsequent specification releases.
