...
- **Define a failure vocabulary.** An operation's `output` describes successful values only. Which outcomes of a binding are successes is its binding specification's concern; failure outcomes have no portable core value representation. This absence of a core error object is not permission to discard source-native failure information: a binding specification defines which observations remain part of its interaction, while an invocation interface or SDK decides how to carry and present them.
...
- **OBI-B-02**: Defines each of the following for the sources and bindings it governs:
  ...
  7. how caller-facing input values and successful output values correspond to the source interaction, including which outcomes are successes, how every source-native semantic observation available from an accepted interaction remains distinguishable when it does not complete successfully, and any context bindings provided at transform positions. Requiring preservation of a failure observation does not make it an operation output and does not prescribe a binding-neutral error schema, invocation interface, or SDK presentation.
