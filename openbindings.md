...
- [10. Conformance](#10-conformance)
  ...
  - [10.5. Conformance conclusions](#105-conformance-conclusions)
...
Where a source carries or points at a binding artifact, an OBI does not replace it. A source may instead address a live surface without a separate artifact. In either case, its binding specification is authoritative over the governed source and interaction. An OBI adds the operation-level overlay that no single concrete interaction alone carries, and does so in a way that survives across multiple protocols.
...
- **Context-free references.** Every OBI-defined document reference is absolute or same-document, so a document's references resolve identically wherever it was obtained (origin, cache, redirect, stdin, or memory). (A source's `content` and a binding's `selector` are interpreted by their binding specification, not by this rule; see [§7. Reference resolution](#7-reference-resolution).)
...
3. **Split authority.** The operation owns the caller-facing value contract. The binding specification owns a source's `content`, a binding's `selector`, and everything needed to act on them: addresses, representations, references, interaction, and success classification. Neither layer overrides the other.
4. **Context-free references.** No OBI-defined reference ([§7](#7-reference-resolution)) resolves against the URI a document was fetched from, so the document model means the same thing however a document was obtained. What a source's `content` and a binding's `selector` mean is their binding specification's to define (invariant 3). OBI assigns no document identity; `name` and `version` are labels.
...
- **Processor**: a tool that takes an OBI document as input. Its baseline capacity is reading (parsing, validating, indexing, or rendering), which it MAY extend with resolving references, validating values, resolving operation names, evaluating transforms, and acting on sources. Rules marked "(all processors)" bind every processor, including one that does no more than read; a processor that exercises further capabilities owes the rules this specification scopes to them, and acting on a source is governed by its binding specification ([§10.1](#101-tool-obligations)).
...
- **Binding**: an author-declared realization of an operation through a source, optionally with a selector its source's binding specification defines. Stored under a key in the document's `bindings` map.
...
- **Caller-facing**: on the operation's side of a binding. Caller-facing values are the ones a caller of the operation sends and receives, before `inputTransform` and after `outputTransform`; the binding specification sees the source-facing values between them ([§5.5](#55-transforms)).
- **Realization**: a concrete way of carrying out an operation's contract through a target. In a document, a binding declares one, as its author's claim ([§5.3](#53-bindings)); a dependency consumes one supplied from elsewhere ([§5.6](#56-dependencies)).
...
```json
{
  ...
  "bindings": {
    ...
    "listTasks.http": {
      ...
      "outputTransform": { "$ref": "#/transforms/apiToTasks" }
    }
  },
  "transforms": {
    "apiToTask": "{ \"id\": task_id, \"title\": task_title, \"done\": is_done }",
    "apiToTasks": "[$.{ \"id\": task_id, \"title\": task_title, \"done\": is_done }]"
  }
}
```

Each transform maps one value to one value. `listTasks` has an array as its output value, so `apiToTasks` maps that one array to another.
...
| Field             | Type                     | Purpose                                                                                           |
| ----------------- | ------------------------ | ------------------------------------------------------------------------------------------------- |
| `selector`        | any JSON value           | Selection of the binding's target. Binding-specification-defined.                                 |
...
Transforms map between operation values and source values when the two differ in shape. They exist so a single operation contract can be reused across bindings whose representations diverge (the operation presents a clean domain model; one source wraps values in envelopes; another names their members differently). Declaring transforms in the OBI keeps shape-translation intent with the interface rather than scattered across per-tool configuration.
...
1. **Pinned language.** OpenBindings 0.2 uses the **JSONata 2.1** language defined by the official repository's [versioned documentation](https://github.com/jsonata-js/jsonata/tree/5d1473277e0022d8580e00f891b12080eb3edd74/website/versioned_docs) at source commit `5d1473277e0022d8580e00f891b12080eb3edd74` (the upstream 2.1.1 release), selecting version 2.1 and its inherited documentation pages. Features introduced after JSONata 2.1 are not part of the 0.2 transform language; a later OpenBindings version may adopt a later target. The language version is bound to the OpenBindings specification version; documents carry no per-document language field. The rendered documentation is informative if it differs from this snapshot. The reference implementation is likewise informative: where its behavior differs from the pinned documentation, the documentation defines the language.
2. **Evaluation.** Evaluation MUST follow the pinned language's syntax and semantics. Conformance is to the documentation, not to any implementation of it: a tool need not use, port, or reproduce the reference implementation, or any other, to conform. The semantics apply to values of the JSON data model ([§5](#5-document-model)): a tool evaluates natively over its own representation of those values and need not serialize them to JSON text. Where a host cannot compute what an expression means for the values it is given, such as a number it cannot hold without changing it or a regular-expression feature its engine lacks, the evaluation is a transform-evaluation failure (clause 4); a tool never returns a different successful value in its place. This specification adds no numeric model to the language's own: where the documentation leaves the precision, range, or text form of numbers open, each host's numbers decide, and a value an expression passes through untouched keeps its exact value.
...
5. **Closed environment.** The evaluation environment is closed over the input value (bound as the JSONata evaluation context), any variable bindings (JSONata's term for named values supplied to an evaluation) the governing binding specification defines for the expression position, and JSONata's standard library. A tool MUST NOT extend it further for document-supplied expressions, neither with variable bindings that reach host state (filesystem, network, environment variables, process state) nor with pure custom functions, since an expression using an extension evaluates on no other tool.
6. **Readings the documentation leaves to its host.** Where the documentation counts or orders characters, a character is a Unicode code point. Objects carry no member order, as in JSON: where a result depends on member order, the order is the host's, and an expression that needs an order sorts explicitly. Regular expressions are ECMA-262 regular expressions as JSONata constructs them: without the `u` flag, with the `i` and `m` flags the documentation allows.
...
| Field          | Type             | Purpose                                                                 |
| -------------- | ---------------- | ----------------------------------------------------------------------- |
| `bindingSpecs` | array of strings | Binding specification identifiers acceptable at this consumption point. |

`operation` references an operation by its key, not by an alias ([OBI-D-19](#102-document-rules)). This is the same key-reference posture bindings use: aliases support name resolution and cross-document correspondence, while same-document relationships carry the canonical key.

When `bindingSpecs` is present, it MUST contain one or more unique binding specification identifiers. The identifiers are exact, opaque, non-empty strings with the semantics of [§6](#6-binding-specifications); array order has no meaning. The list is an **any-of constraint**: a binding considered for this dependency meets the declared binding-family constraint only when its referenced source's `bindingSpec` exactly equals at least one listed identifier. When `bindingSpecs` is absent, the dependency declares no binding-family constraint. Absence does not mean that every binding is usable by a particular runtime, and presence does not assert that an implementation of any listed specification is installed or available.
...
A source names the authority that gives it meaning. The `bindingSpec` value is a **binding specification identifier**: an exact, opaque, non-empty string denoting one stable semantic definition — a **binding specification** — under one defining authority ([OBI-B-01](#104-binding-specification-rules)). Dependencies use the same identifiers in `bindingSpecs` to constrain acceptable realization families; listing an identifier does not make the dependency a source or target.
...
and bindings that name it. No external artifact, protocol, or community becomes
authoritative merely because a source resembles or identifies its format. A
binding specification may incorporate another authority on stated terms,
subset or extend it, override it, use only its representation syntax, define an
artifact and interaction model of its own, or govern an artifactless live
surface. That relationship is part of the binding specification's stable
...
- **Carriage** ([§5.4](#54-sources)): a source's optional `content`, any JSON value, to which this specification assigns no meaning.
- **Selection** ([§5.3](#53-bindings)): a binding's optional `selector`, any JSON value, to which this specification assigns no meaning.
- **Values** ([§5.1](#51-operations), [§5.5](#55-transforms)): each value crossing an operation boundary is one JSON value under a per-value contract; `output` describes successful values only; the binding specification sees the source-facing value, after `inputTransform` and before `outputTransform`; and it may supply variable bindings at transform positions.

[OBI-B-02](#104-binding-specification-rules) is the completion of that handoff: at each of these points, the meaning this specification leaves open is what a binding specification defines. Its four items are what a binding specification owes in return, not further provisions: the third, how a target and its interaction are identified, rests on no provision here and is wholly the binding specification's. The rest of this specification is the document's and the tools' business: the spelling of `bindingSpec`, preference signals, operation identifiers and aliases, dependencies, schema semantics, reference resolution, and the transform language. These three are the only provisions [§8.1](#81-openbindings-field-specification-version) holds stable for binding specifications; a binding specification that relies on any other provision relies on something this specification may change. A binding specification that also describes how documents are synthesized from its artifacts writes whole documents and may reach any part of the model in doing so; that reach belongs to synthesis, not to binding meaning.
...
1. **A schema `$ref`** (on an operation `input`/`output`, in the `schemas` map, or nested as a subschema): a same-document fragment in JSON Pointer form (a bare `#`, or `#` followed by an [RFC 6901](https://www.rfc-editor.org/rfc/rfc6901) pointer) or an absolute URI. In this specification an absolute URI is a URI with a scheme (RFC 3986 §3), and it may carry a fragment; RFC 3986's `absolute-URI` production, which excludes fragments, is not what is meant.
...
2. **A named-transform `$ref`** in `bindings[*].inputTransform`/`bindings[*].outputTransform`: a same-document fragment in literal form resolving into the `transforms` map ([OBI-D-10](#102-document-rules)).

A source's `content` and a binding's `selector` are **not** OBI-defined document references. They are interpreted by the governing binding specification ([§5.3](#53-bindings), [§5.4](#54-sources)) and are exempt from the absolute-or-same-document requirement.
...
- **URIs as attack vectors.** Addresses a binding specification reads from a source's `content` or a binding's `selector`, and schema `$ref` values, may resolve to arbitrary network endpoints, including internal or link-local addresses such as `http://169.254.169.254/...` or `file:///etc/passwd`. Unrestricted dereferencing inherits SSRF and exfiltration exposure.
...
- **Regular-expression cost.** Schema `pattern` and `patternProperties` values, and regular expressions in transforms, run on the evaluating tool's regular-expression engine; a backtracking engine can take exponential time on crafted input.
- **Schema `$ref` cycles.** Permitted by [§7](#7-reference-resolution); naive resolvers can exhaust the stack or loop indefinitely.
- **Transforms as executable code.** Transforms are JSONata source. Untrusted documents can embed expressions designed to run without bound when evaluated. On a conforming tool the exposure is bounded to computation: the closed evaluation environment ([OBI-T-10](#103-tool-rules)) bars document-supplied expressions from host state, apart from what JSONata's standard library itself reads: the evaluation's clock (`$now` and `$millis`, which return one value throughout an evaluation) and pseudo-random numbers (`$random`). Closure is normative because host-reaching variable bindings would break transform portability before they broke security; bounding evaluation time and memory remains per-tool policy.
...
- **Linear-time regular expressions**, or time limits on matching, for schema patterns and transform regular expressions.
...
A tool's obligations follow the capabilities it exercises, not a fixed class. A tool that only parses, validates against the document rules, indexes, or renders OBI documents owes the rules marked _all processors_. A tool that also resolves references, validates values against operation contracts, resolves operation names, or evaluates transforms additionally owes the rules scoped to those activities. Acting on a source is governed by its binding specification, not by a rule here; a tool claiming support for a binding-specification identifier claims support for that specification as published ([§10.4](#104-binding-specification-rules)). Invoking a binding, by itself, triggers no rule in this specification (invariant 2).
...
- **OBI-D-05**: Carries only absolute or same-document OBI-defined references, in the forms of [§7](#7-reference-resolution): every schema `$ref` a same-document JSON Pointer fragment in literal form or an absolute URI; every schema `$id` at an OBI position absolute, and no two schema resources the document embeds declaring the same `$id` once each is resolved; every named-transform `$ref` a same-document fragment in literal form; no `$anchor`-fragment references, `$dynamicRef`, or `$dynamicAnchor` at OBI positions (all per [§7](#7-reference-resolution), including the internal-business scope of schema resources declaring their own `$id`). Every URI-form reference is well-formed per [RFC 3986](https://www.rfc-editor.org/rfc/rfc3986) §4.1.
...
- **OBI-T-02** (all processors): Ignores unknown fields not defined by this specification. "Fields" are the properties of OBI-defined objects (the document root; operation, dependency, source, binding, and example objects; and the `$ref` object form of `inputTransform`/`outputTransform`); property names inside JSON Schema objects are schema content, out of scope for this rule, mirroring OBI-D-03. An unknown field whose name does not begin with `x-` also makes the document non-conformant ([OBI-D-02](#102-document-rules), [§12](#12-extensions)); a tool processing the document ignores it rather than failing. Tools SHOULD surface diagnostics for unknown non-`x-` fields.
...
- **OBI-B-02**: Defines each of the following for the sources and bindings it governs:
  ...
  2. the accepted values and meaning of `selector`, and the meaning of its absence;
  3. how the binding target and its interaction are identified, including any part a processor's environment plays;
  4. how caller-facing input values and successful output values correspond to the source interaction, including which values it produces are successful output values, and any variable bindings it provides at transform positions ([§5.5](#55-transforms) clause 5).

  The enumerated items are the completeness test; their purpose is that two independent implementations agree on what this specification hands over: the meaning of a source's `content`, a binding's `selector`, its target and interaction, and the values that cross the operation boundary. An item left undefined means the specification does not satisfy OBI-B-02; behavior an implementation chooses for the gap is implementation-defined, not the identifier's meaning ([§6](#6-binding-specifications)).
...
### 10.5. Conformance conclusions

A document is objectively conformant or non-conformant under this specification (invariant 5); **conformance undetermined** is not a third document state but a validator's conclusion that its evidence is insufficient. A tool reporting an overall conclusion uses three meanings ([OBI-T-17](#103-tool-rules)):
