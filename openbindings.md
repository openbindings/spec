...
- **OBI-D-05**: Carries only absolute or same-document OBI-defined references. Every `sources[*].location` is an absolute URI or a format-defined absolute address (never a relative reference); every schema `$ref` is a same-document fragment or an absolute URI; every schema `$id` is absolute; every named-transform `$ref` in `bindings[*].inputTransform`/`bindings[*].outputTransform` is a same-document fragment. All such references are well-formed per [RFC 3986](https://www.rfc-editor.org/rfc/rfc3986) §4.1. `bindings[*].ref` is interpreted by the binding format and is out of scope for this rule (see [§10. Reference resolution](#10-reference-resolution)).
