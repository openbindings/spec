...
2. A schema `$ref` (whether on an operation `input`/`output`, in the `schemas` map, or nested as a subschema within any of these): a same-document fragment in JSON Pointer form (a bare `#` or `#` followed by an [RFC 6901](https://www.rfc-editor.org/rfc/rfc6901) pointer) or an absolute URI. Plain-name (`$anchor`) fragments are not used at OBI positions: the `schemas` map is the document's named-schema mechanism (`#/schemas/<name>`). A schema `$id`, when present, is absolute.
