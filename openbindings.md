...
- **Source**: a reference to a binding artifact, identified by a format token and a location, embedded content, or both (see [§6.4. Sources](#64-sources)).
...
Whether one OBI is compatible with another is a matter of tool-defined comparison and matching semantics; this spec defines neither. Cross-document correspondence is claimed by name adoption: an operation **satisfies** a shared contract's operation by carrying that contract's operation name as its key or an alias. The mechanism is detailed in [§6.1. Operations](#61-operations).
...
Operations do not prescribe invocation pattern. Whether an operation is request/response, streaming, bidirectional, or pub/sub is determined by the binding, not by the operation definition. The operation model is shaped to accommodate any of these patterns that its binding implements. When a binding carries more than one value (a stream), the `input`/`output` schemas describe **one value** of the stream, not a collected aggregate; validation applies to each value individually as it crosses the operation boundary ([OBI-T-08](#143-tool-rules)).
...
| `idempotent` | boolean | Author-attested claim: `true` asserts multiple invocations with the same input produce the same observable state; `false` asserts they may not; absent claims nothing. |
...
Common uses of `aliases`: a prior name kept for continuity after a rename, a vendor-specific name some consumers look up by, or a shared-contract name that makes this operation addressable as the implementation of a published interface (e.g., `tasks.create` on a `createTask` operation). The last is one way an operation **satisfies** a contract operation (the cross-document correspondence of [§5. Terminology](#5-terminology)): it adopts the contract's operation name as an alias. An operation can equally satisfy by using the contract's name as its key; key and alias are equal identifiers for resolution, so the alias form matters only when the operation keeps a different primary key. The name is an ordinary identifier. The spec attaches no verification, ownership, or trust semantics to it; whether an alias legitimately names a published contract is a registry and tooling concern, not a property the document can assert or a tool can confirm from the document alone.
...
| `idempotent` | author-attested | consumers MAY rely on it (either polarity); its semantic truth is not mechanically verified, and absence asserts nothing |
...
`examples` holds author-supplied sample input/output pairs for an operation. The spec defines only the structural shape; consumers such as generated docs, SDK codegen, test harnesses, and agent few-shot material apply them as they see fit. Within any example, a provided `input` MUST validate against the operation's `input` schema when that schema is specified, and a provided `output` MUST validate against the operation's `output` schema when that schema is specified. An example's `input`/`output` members are instance values, not schemas, so the absent-equals-`null` equivalence of the operation's schema fields does not apply here: an explicitly `null` example member is a provided value (JSON `null`) and is validated — otherwise an operation whose `output` schema is `{"type": "null"}` could carry no example — while an absent member is simply not provided.
...
For new binding formats whose artifact is a JSON document and whose addressing scheme is the format's own design choice (rather than inherited from an upstream protocol like MCP or gRPC), the project's format authoring guidance (`formats/README.md` in the specification repository) recommends [JSON Pointer (RFC 6901)](https://www.rfc-editor.org/rfc/rfc6901) as the `ref` syntax. That guidance — like all format authoring convention — is informational: this spec binds documents and tools, not future format specifications.
...
| Field | Type | Purpose |
|---|---|---|
| `location` | string | Absolute URI or format-defined absolute address: the external binding artifact, or — per the format's conventions — the live service it describes. |
...
When `content` is a string, it holds the UTF-8 source text of the binding artifact (for example, a `.proto` file's source, or a KDL document). When `content` is an object, it holds the parsed JSON representation of the artifact. Binary artifacts MUST be carried via `location` rather than embedded; `content` is for textual and JSON-structured forms only ([OBI-D-14](#142-document-rules)). Format communities determine which textual or structural representation is canonical for a given format; a format may accept only one form, or both.

What a `location` names is the binding format's concern: for most formats it is the artifact's own address (an OpenAPI document's URL); for a format whose artifact describes a live service that can serve its own description, it is the service's format-defined address (a gRPC `host:port`), from which the artifact is discoverable. `content` always carries the artifact itself. The meaning of a source carrying both therefore follows from the format's conventions ([OBI-T-15](#143-tool-rules)). Where `location` names the artifact, a processor MUST interpret the binding artifact from `content`, and the location is provenance and documentation only (unless the processor explicitly chooses to refresh or compare against the external artifact). Where `location` addresses the service, there is no competition to resolve: `content` pins the artifact and `location` remains the invocation target. A format whose conventions do not address the pairing is content-authoritative.

Embedded `content` carries no fetch location of its own, so OBI defines no base URI for references internal to the embedded artifact (a relative `$ref` in an embedded OpenAPI document, a protobuf `import`). Embedded `content` MUST therefore be self-contained: every reference internal to the artifact resolves without an external base (for example, bundled — or a compiled form such as a protobuf `FileDescriptorSet`, carried in a textual or JSON encoding the format's community blesses, never as raw binary, per [OBI-D-14](#142-document-rules)) ([OBI-D-15](#142-document-rules)). An artifact that relies on a base URI to resolve its own references is carried via `location` instead, where the format resolves those references per its own rules. Whatever a co-present `location` names, it is never a resolution base for references internal to embedded `content`; this keeps a document with embedded `content` resolvable identically everywhere, consistent with the context-free guarantee of [§10. Reference resolution](#10-reference-resolution). A consequence worth noting (non-normative): when `content` is present, `location` never reflects the artifact a processor interprets — a document-address location is the artifact's origin, a service-address location is the wire target — so provenance, signing, and review attach to the embedded `content` itself. Authors may set both when they want a self-contained document (the embedded `content` makes processing independent of network fetches) while preserving the canonical origin URL for documentation, provenance, or downstream tools that prefer a live fetch — or, for a service-addressed format, to carry the pinned contract and the invocation target in one source.

**Format tokens.** The `format` field is a string that identifies the binding specification for a source. Format tokens are community-extensible; what characters appear in them and how different strings relate to one another (equivalence, compatibility, ordering) is determined by each format's own community. For new format tokens, the project's format authoring guidance (`formats/README.md`) records the recommended convention — `<name>@<version>` (e.g., `openapi@3.1`, `mcp@2025-11-25`), with sources declaring exact versions and tools declaring supported ranges. This spec does not maintain a registry of format tokens; authority over a format's conventions lies with that format's own specification or, in its absence, its implementer community.
...
A transform is a [JSONata](https://jsonata.org/) expression string. A tool that evaluates transforms (see [§14.1. Tool obligations](#141-tool-obligations)) MUST do so according to [JSONata 2.0](https://docs.jsonata.org/); any implementation may provide its own JSONata 2.0 evaluator. The evaluation environment is closed over the input value (bound as the JSONata evaluation context — the value the expression's paths navigate from, as in the examples above), any bindings the governing format specification itself defines for the expression position being evaluated (the pattern operation-graph sets by defining `$input` for its own node expressions; no current format defines bindings for a binding's `inputTransform`/`outputTransform`), and JSONata's standard library: a tool MUST NOT extend it further for document-supplied expressions — neither with bindings that reach host state (filesystem, network, environment variables, process state) nor with pure custom functions, since an expression using an extension evaluates on no other tool. JSONata as referenced provides no host access; this rule bars adding any extension, so the same transform over the same input behaves identically on every conforming tool (up to the standard library's own non-determinism, below).
...
A JSONata expression can also evaluate to no value at all — JSONata's *undefined*, the result of paths into absent data — which is distinct from evaluating to `null`, a JSON value like any other. A `null` result is a result: it crosses the operation boundary and is validated as a value. For an undefined result, a tool SHOULD treat the evaluation as an expression failure — an invocation error for that operation — rather than emit a value the expression never produced or silently skip the emission. Beyond that recommendation, undefined handling is tool-defined, and it is a named non-portable axis: a portable document does not rely on any particular undefined-result behavior, and its transforms produce a defined value (possibly `null`) for every value they can be applied to — caller inputs the operation admits and source outputs the binding can yield.
...
A transform expression's parse-validity is not a document rule: no rule in [§14.2. Document rules](#142-document-rules) requires a document's JSONata expressions to parse, and a malformed expression surfaces at evaluation time as an invocation error for the operations whose bindings use it — the same lane as any other evaluation failure. A validator MAY still parse expressions and surface diagnostics as warnings; refusing the document on that ground is not conformance behavior.
...
- Clients SHOULD send `Accept: application/vnd.openbindings+json, application/json` to signal media-type preference. Servers MAY content-negotiate on this header but MUST NOT refuse a request that lacks it when they otherwise serve an OBI at this path. Content negotiation at this path is ordinary HTTP ([RFC 9110](https://www.rfc-editor.org/rfc/rfc9110)), and this spec does not restate its mechanics. What it pins is the default: the OBI is the path's default representation, and a request without an `Accept` header receives it. (A publisher may therefore serve, for example, an HTML page to a browser that prefers one; a client sending the `Accept` header above always receives the OBI.)
...
1. `sources[*].location`: an absolute URI, or a format-defined absolute address that needs no base URI to interpret (for example, a gRPC `host:port`). It is never a relative reference. A source MAY also embed its artifact as `content`, with or without a `location` (the pairing is the binding format's concern; see [§6.4. Sources](#64-sources)).
2. A schema `$ref` (whether on an operation `input`/`output`, in the `schemas` map, or nested as a subschema within any of these): a same-document fragment in JSON Pointer form (a bare `#` or `#` followed by an [RFC 6901](https://www.rfc-editor.org/rfc/rfc6901) pointer) or an absolute URI. Plain-name (`$anchor`) fragments are not used as same-document references at OBI positions: the `schemas` map is the document's named-schema mechanism (`#/schemas/<name>`). The dynamic pair (`$dynamicRef`/`$dynamicAnchor`) does not appear at OBI positions at all: a `$dynamicRef` whose resolution engages no dynamic anchor behaves exactly as `$ref` (write `$ref`), and any form that does engage one resolves against the runtime dynamic scope — resolution that depends on the evaluation path is precisely the context-dependence this clause exists to exclude, and `$dynamicAnchor` would be a second named-schema mechanism competing with the `schemas` map, exactly as `$anchor` would. A fragment carried on an absolute URI belongs to the external resource it addresses, and a `$ref` within a schema that declares its own `$id` resolves against that resource's base per JSON Schema — both are out of scope for this clause, the latter exactly as for [OBI-D-16](#142-document-rules); a schema resource declaring its own `$id` may likewise use the dynamic pair internally, per the same scope rule. A schema `$id` at an OBI position, when present, is absolute; a nested `$id` inside a schema that declares its own `$id` resolves against that resource's base per JSON Schema 2020-12 and MAY be relative — that resource's internal business, per the same scope rule.
...
Schema `$ref` and `$id` resolution follows [JSON Schema 2020-12](https://json-schema.org/draft/2020-12): a `$id`, when present, establishes the schema resource and the base URI for the `$ref`s inside it. OBI does not override that; it only constrains the forms (OBI-D-05: each schema `$ref` a same-document JSON Pointer fragment or an absolute URI, each `$id` absolute), which is what keeps schema resolution independent of where the OBI document was fetched. A same-document fragment `$ref` is a JSON Pointer and uses [RFC 6901](https://www.rfc-editor.org/rfc/rfc6901) escapes (`~0` for `~`, `~1` for `/`). The fragment is the pointer's URI-fragment representation per RFC 6901 §6: evaluation percent-decodes the fragment first, then evaluates the result as a JSON Pointer — `#/schemas/T%61sk` addresses the `schemas` key `Task`, exactly as `#/schemas/Task` does. An absolute `$ref` resolves within the document when it matches the `$id` a schema embedded in the document declares (standard JSON Schema 2020-12 identity resolution — no fetching involved); otherwise it addresses an **external** schema a tool fetches. A tool MAY decline to fetch external schemas, and a document whose `$ref`s all resolve within it (same-document fragments and embedded-`$id` matches) is resolvable with no network access at all, as the OpenBindings project's published interfaces are. Declining has a fixed invocation-time consequence ([OBI-T-07/OBI-T-08](#143-tool-rules)): a governing schema the tool cannot fully resolve validates nothing — the invocation fails rather than passing partially.

For schemas embedded in an OBI document, the initial base URI for resolving same-document fragments is the OBI document root, not the URI the document was fetched from. A same-document fragment `$ref` (such as `#/schemas/Task`) is therefore evaluated as an [RFC 6901](https://www.rfc-editor.org/rfc/rfc6901) JSON Pointer from that root, and a bare `#` addresses the root, until resolution enters a schema that declares its own `$id` (which, per JSON Schema 2020-12, establishes a new base URI for the `$ref`s within it). JSON Schema 2020-12 leaves the resolution context of schemas embedded in a non-schema document to the embedding application; OBI fixes that context here so a `#/schemas/...` reference resolves identically across tools. A same-document fragment `$ref` also actually resolves in a conformant document: internal referential integrity is a document rule ([OBI-D-16](#142-document-rules)), as it is for binding and transform references (OBI-D-08/09/10), while external `$ref` resolution remains an evaluation-time concern. A consequence (informative): a tool that extracts an operation's `input` or `output` subschema and resolves it through a JSON Schema implementation in isolation must preserve the OBI document as the resolution scope (for example, by bundling or registering it), or the same-document fragment will not resolve.
...
Refusal runs downward as well: a tool MUST refuse documents declaring a version below the minimum it supports (pre-1.0: any unsupported lower minor). Acceptance without the version's own rules is misreading — pre-1.0 minors may change field semantics in either direction, so a tool processing an older document under newer rules can silently invert meanings just as surely as the reverse. A tool declares the version range it supports, and refuses anything outside it — where "outside" is at the refusal-trigger granularity above: a differing major in either direction, or, while pre-1.0, a differing minor in either direction. A differing minor within a supported major (post-1.0) is not a refusal trigger: newer minors only add optional fields and existing fields never change meaning, so a higher minor is read by ignoring its additions and a lower minor is read under rules that still hold.
...
- **Transforms as executable code.** Transforms are JSONata source code. Untrusted documents can embed expressions designed to run without bound when evaluated. On a conforming tool the exposure is bounded to computation: [OBI-T-10](#143-tool-rules)'s closed evaluation environment bars document-supplied expressions from host state.
...
Mitigation strategies, such as scheme allow-lists, size caps, cycle detection, and HTTPS enforcement, are processor concerns. The spec does not mandate mitigation policy — with one semantics-grounded exception: the transform evaluation environment is closed normatively ([OBI-T-10](#143-tool-rules)), because host-reaching bindings would break transform portability before they broke security. [§13.1. Recommended mitigations (informative)](#131-recommended-mitigations-informative) lists the policy categories tools typically consider.
...
- **Network-range restrictions**. Refusing to dereference URIs resolving to link-local (`169.254.0.0/16`, `fe80::/10`), loopback (`127.0.0.0/8`, `::1`), private (RFC 1918, and IPv6 unique-local `fc00::/7`), or carrier-grade NAT (`100.64.0.0/10`) address ranges by default, with explicit operator opt-in for environments that legitimately need them. A complete treatment checks the IANA special-purpose address registries and normalizes IPv4-mapped IPv6 forms (`::ffff:10.0.0.1`) before comparison, and applies the check after DNS resolution, per redirect hop.
- **Size caps** on fetched documents, schemas, and binding artifacts.
- **Timeouts** on fetches and on transform evaluation.
- **JSONata resource isolation**. Host-state access is already barred normatively ([OBI-T-10](#143-tool-rules)'s closed evaluation environment); what remains per-tool is bounding evaluation time and memory for document-supplied expressions.
...
A tool self-declares the capabilities it implements in its documentation or metadata. The openbindings project does not maintain a registry of conformant tools; there is no central registration. A conformance test corpus is published as reference material (not part of this specification) and can check a declaration empirically where it provides fixtures; a rule without fixtures is no less binding, it simply rests on self-declaration alone.

### 14.2. Document rules

Document rules bind the document; verifying a clause takes capabilities. Checking OBI-D-01's duplicate clause requires a duplicate-detecting parse; OBI-D-05's colon-bearing format-defined addresses and OBI-D-13/OBI-D-15 take per-format knowledge (OBI-D-05's relative-reference clause does not — see that rule's verification note); OBI-D-11 takes fully resolving the operation schemas, which for a schema carrying an external `$ref` means fetching. A validator that lacks a capability a clause requires leaves that clause **unverified** rather than failing the document: conformance is a property of the document, not of any validator's capabilities, and unverified is not non-conformant (the partial-verification posture of [§8. Binding sufficiency](#8-binding-sufficiency)). Per-rule verification notes below are instances of this posture.

A conformant **OBI document**:

- **OBI-D-01**: Is valid UTF-8 encoded JSON per [RFC 8259](https://www.rfc-editor.org/rfc/rfc8259). Duplicate JSON object keys within any object make the document invalid. A leading byte-order mark (U+FEFF) makes the document invalid: RFC 8259 §8.1 forbids adding one, interoperable-JSON practice (I-JSON, [RFC 7493](https://www.rfc-editor.org/rfc/rfc7493)) excludes it, and tolerating it would let two parsers disagree over the same bytes. Verification note (informative): RFC 8259 leaves duplicate-name handling unspecified, and most JSON parsers silently keep one value rather than reporting the duplicate, so checking this clause requires a duplicate-detecting parse. A validator whose parser cannot surface duplicates leaves the clause unverified rather than failing the document; this is the same partial-verification posture as [§8. Binding sufficiency](#8-binding-sufficiency).
...
- **OBI-D-05**: Carries only absolute or same-document OBI-defined references. Every `sources[*].location` is an absolute URI or a format-defined absolute address (never a relative reference); every schema `$ref` is a same-document fragment in JSON Pointer form ([RFC 6901](https://www.rfc-editor.org/rfc/rfc6901); plain-name `$anchor` fragments are not used at OBI positions, and a `$ref` within a schema declaring its own `$id` is that resource's internal business, per the same scope rule as OBI-D-16) or an absolute URI; every schema `$id` at an OBI position is absolute (a nested `$id` within a schema declaring its own `$id` resolves against that resource's base per JSON Schema 2020-12 and is that resource's internal business, per the same scope rule); every named-transform `$ref` in `bindings[*].inputTransform`/`bindings[*].outputTransform` is a same-document fragment. `$dynamicRef` and `$dynamicAnchor` do not appear at OBI positions: dynamic resolution follows the runtime dynamic scope rather than the document (see [§10. Reference resolution](#10-reference-resolution), clause 2), and within a schema declaring its own `$id` both are that resource's internal business, per the same scope rule as above. Every URI-form reference here is a well-formed URI reference per [RFC 3986](https://www.rfc-editor.org/rfc/rfc3986) §4.1: absolute-URI `location`s, absolute schema `$ref`/`$id`, and same-document fragments. A format-defined absolute address used as a `location` (for example, a gRPC `host:port`) is well-formed per its own format, not RFC 3986, and is exempt from this clause. `bindings[*].ref` is interpreted by the binding format and is out of scope for this rule entirely (see [§10. Reference resolution](#10-reference-resolution)). Verification note (informative): the relative-reference clause is decidable without format knowledge. A `location` with no `:` before its first `/`, `?`, or `#` cannot carry a scheme and is a relative reference in form ([RFC 3986](https://www.rfc-editor.org/rfc/rfc3986) §4.2), rejectable by any validator — `./openapi.json` violates this rule everywhere; a consequence is that a bare name like `example.com` is relative-reference form and not a conformant location (a format wanting DNS-name or bare-host addresses carries a scheme or another colon-bearing form, as `host:port` does). Only a colon-bearing string that is not a well-formed URI takes per-format knowledge — it may be a format-defined absolute address — and a validator without that format's knowledge leaves that case unverified, per the posture above.
...
- **OBI-D-11**: Has every example's provided `input` validating against its operation's `input` schema, and every example's provided `output` validating against its operation's `output` schema, when the respective schema is specified. An explicitly `null` example member is a provided value and is validated; only an absent member is unprovided (see [§6.1. Operations](#61-operations)).
...
- **OBI-D-14**: Carries only textual or JSON-structured forms in `sources[*].content`; binary artifacts ride `location`. (See [§6.4. Sources](#64-sources).)
- **OBI-D-15**: Has every embedded `sources[*].content` self-contained: no reference internal to the embedded artifact requires an external base to resolve. Verification is per-format knowledge, the same partial-verification posture as [OBI-D-13](#142-document-rules). (See [§6.4. Sources](#64-sources).)
- **OBI-D-16**: Has every schema `$ref` that is a same-document fragment resolving, from the OBI document root per [§10. Reference resolution](#10-reference-resolution), to a location that exists within the document. A `$ref` within a schema that declares its own `$id` resolves against that resource's base and is out of this rule's scope; an absolute-URI `$ref` is out of scope too unless it matches an embedded schema's `$id` — external resolution is an evaluation-time concern, and "external" means not resolvable within the document per [§10](#10-reference-resolution). (`$dynamicRef` does not participate in this rule: it is barred at OBI positions by OBI-D-05, and within an embedded-`$id` resource it is that resource's internal business.) Together with [OBI-D-08](#142-document-rules), [OBI-D-09](#142-document-rules), and [OBI-D-10](#142-document-rules), this completes one posture for the document's internal references: a same-document reference is document integrity, offline-decidable by any validator; only external resolution is deferred to evaluation.
...
- **OBI-T-02** (all processors): Ignores unknown fields that are not defined by this specification. "Fields" here are the properties of OBI-defined objects (the document root; the operation, source, binding, and example objects; and the transform-reference object, the `$ref` object form of `inputTransform`/`outputTransform`); property names inside JSON Schema objects (at `input`, `output`, and `schemas` positions) are schema content, not OBI fields, and are out of scope for this rule, mirroring [OBI-D-03](#142-document-rules). Their handling follows [§6.2. Schemas](#62-schemas), and tool diagnostics for uninterpreted schema keywords are [OBI-T-05](#143-tool-rules)'s concern. Tools SHOULD surface diagnostics (warnings) for unknown non-`x-` OBI fields to help catch typos.
...
- **OBI-T-04** (all processors): Applies the version acceptance and refusal rules of [§11.1. `openbindings` field (spec version)](#111-openbindings-field-spec-version). A tool MUST refuse documents declaring a higher major `openbindings` version than it supports; while the spec is pre-1.0 (major version 0), this refusal extends to higher minor versions. A tool MUST likewise refuse documents declaring a lower major version than it supports; while the spec is pre-1.0, this downward refusal extends to any unsupported lower minor. Post-1.0, a lower minor within a supported major is accepted rather than refused (existing fields never change meaning, so an older document reads under rules that still hold). A version outside the tool's supported range at this granularity — in either direction — is refused rather than processed under the wrong rules. Versions are compared by [Semantic Versioning 2.0.0](https://semver.org/spec/v2.0.0.html) precedence: build metadata is ignored, and a higher patch within a supported major/minor is not a refusal trigger (it is accepted as non-breaking). A prerelease (for example `0.2.0-rc.1`) sorts below its release and is a distinct, potentially incompatible draft; a tool MUST NOT accept a prerelease unless it declares support for that specific prerelease.
...
- **OBI-T-07** (applies when invoking): A tool MUST validate caller-provided input against the operation's `input` schema when that schema is specified. Validation is against the fully resolved schema, judged statically over the whole governing schema before validating (an unresolvable branch fails the invocation even when the value would not reach it; the governing schema is everything statically reachable from the operation's schema root — keyword subschemas and reference targets, transitively — so a schema present in the document but unreachable from that root does not participate in the judgment, and a dangling same-document reference there is [OBI-D-16](#142-document-rules)'s document-level concern, not an invocation failure): a tool that cannot fully resolve it (an external `$ref` it declines or fails to fetch; "external" per [§10](#10-reference-resolution)) MUST treat the invocation as failed — an invocation error naming the unresolvable reference — rather than validate partially. If the binding declares an `inputTransform`, validation MUST occur before transform application. A validation failure is an invocation error for that operation; it does not render the document non-conformant.
- **OBI-T-08** (applies when invoking): When the operation's `output` schema is specified, a tool MUST validate the operation's result against the `output` schema. The operation's result is the value an invocation yields as its success outcome; which outcomes of a binding constitute success is the binding format's concern. Failure outcomes are invocation errors, not results, and are not subject to output validation. If the binding declares an `outputTransform`, the result to validate is the value produced by evaluating the transform; otherwise it is the source's output as received. As with OBI-T-07, validation is against the fully resolved schema: a tool that cannot fully resolve it MUST raise an invocation error naming the unresolvable reference rather than validate partially. The static resolvability judgment (both directions, OBI-T-07's and this rule's) SHOULD precede driving the binding, so an invocation doomed by an unresolvable schema causes no side effects; sequencing is internal to the tool, but a side-effecting call followed by an output-schema refusal is variance a user observes. A validation failure is an invocation error for that operation; it does not render the document non-conformant. For a streaming binding that carries more than one value, OBI-T-07 and OBI-T-08 apply to each value individually as it crosses the operation boundary (per input item before send, per output item as received or transformed), not to a collected aggregate of the stream; the `input`/`output` schema describes one such value, and heterogeneous streams express their variants with JSON Schema alternation (`oneOf`/`anyOf`).
- **OBI-T-09** (applies when selecting among multiple bindings for the same operation): Selection applies only among the *candidate* bindings the tool can act on. A tool need not include in the candidate set a binding whose source format it does not support, whose `ref` it cannot resolve, or that it declines for local policy. Whether the candidate set is determined eagerly (every binding assessed up front) or lazily (assessed as needed, falling back on failure) is tool-defined; the tier and preference rules below govern whichever set the tool considers. Among the candidates, a tool MUST prefer non-deprecated bindings over deprecated ones. A deprecated binding with any `preference` value is ranked after every non-deprecated binding. Within each tier, tools SHOULD prefer higher effective `preference` values, where a binding's effective `preference` is its own `preference` when present, otherwise its source's `preference` when present, and otherwise 0 (binding-level `preference` overrides the source default rather than combining with it, per [§6.3. Bindings](#63-bindings)); an absent preference is thus the neutral baseline of 0, and bindings that share the highest effective `preference` are ordered tool-defined per [§6.3](#63-bindings). Tools MAY apply additional selection tactics (latency, cost, protocol preference) after the tier rule has been applied.
- **OBI-T-10** (applies when evaluating transforms): Evaluates declared transforms according to [JSONata 2.0](https://docs.jsonata.org/). Any implementation may provide its own JSONata 2.0 evaluator. The evaluation environment is closed: a tool MUST NOT extend it for document-supplied expressions — neither with bindings that reach host state (filesystem, network, environment variables, process state) nor with pure custom functions; bindings the governing format specification itself defines are part of the environment, not extensions (see [§6.5. Transforms](#65-transforms)).
...
- **OBI-T-13** (applies when serving an OBI at the discovery URI): When a service serves an OBI at `/.well-known/openbindings`, it MUST answer a `GET` with `200 OK` and a body that is a valid OBI document, and MUST NOT refuse the request solely because the `Accept` header is absent or omits an OBI media type. Content negotiation at this path is ordinary HTTP (see [§7.1](#71-response-contract)); the MUST above pins the default representation, not a ban on negotiated alternatives. The response `Content-Type` SHOULD be `application/vnd.openbindings+json`. Where the service would serve the request but publishes no OBI at the path, the response is `404 Not Found`; an endpoint that gates discovery behind authentication or authorization MAY instead respond `401`/`403`. A discovery endpoint intended for public consumption SHOULD set permissive CORS response headers (for example, `Access-Control-Allow-Origin: *`). (See [§7. Discovery](#7-discovery).)
- **OBI-T-14** (applies when fetching an OBI from the discovery URI): A client fetching `/.well-known/openbindings` MUST accept a `200 OK` response served as `application/json` as well as one served as `application/vnd.openbindings+json`, SHOULD send `Accept: application/vnd.openbindings+json, application/json`, and SHOULD follow `3xx` redirects subject to its own redirect policy and limits. A client MAY treat any response that is not a `200 OK` carrying a valid OBI (a `404`, another `4xx`/`5xx`, or a `200` whose body is not a valid OBI) as "no OBI published at this path." That MAY does not extend to a `200 OK` carrying an OBI document whose declared `openbindings` version the client refuses per [OBI-T-04](#143-tool-rules): that outcome is a version refusal — the publisher's document exists but cannot be processed under the client's supported range — and collapsing it into "no OBI published" would discard the one diagnostic the user can act on; how the refusal is surfaced is tool-defined. (See [§7. Discovery](#7-discovery).)
- **OBI-T-15** (applies when acting on sources): Interprets a source carrying both `location` and `content` per the binding format's conventions: where the format's location names the artifact, the embedded `content` is the authoritative artifact and the location is provenance; where the format's location addresses the live service, `content` carries the artifact and `location` remains the invocation target. For a format whose conventions do not address the pairing, a tool MUST treat the source as content-authoritative. (See [§6.4. Sources](#64-sources).)
...
- **OBI-T-10** is a MUST because JSONata 2.0 is the spec's mandated transform interchange language; a tool that evaluated transforms in any other language would silently produce different results from a conforming one, breaking the portability §6.5 commits to. The closed-environment clause is the same commitment from the environment's side: a host-reaching binding makes a transform's value depend on where it runs — the identical portability break arriving as an extension — and it is also what keeps document-supplied expressions inert against the host (see [§13. Security considerations](#13-security-considerations)).
...
- **OBI-T-15**'s default (content-authoritative absent a format convention) is a MUST because a source whose pairing no one defines must still mean the same thing to every tool; the per-format interpretations themselves stay with the formats per the [§2. Scope principle](#2-scope-principle), exactly as `ref` syntax does (OBI-T-06).
...
This spec does not define tool behavior beyond these minimum conformance rules. Behavioral conventions (comparison, transform runtime policy such as error handling and resource limits, credential resolution, and the like) are documented per-tool. The transform *language* itself is specified and its evaluation environment closed (JSONata 2.0, OBI-T-10); what stays per-tool is the runtime around it.
...
"Object location" here means the properties of OBI-defined objects: the document root, the operation, source, binding, and example objects, and the transform-reference object (the `$ref` object form of `inputTransform`/`outputTransform`) — the same positions [OBI-T-02](#143-tool-rules) enumerates. Keys inside the document's maps (`operations`, `sources`, `bindings`, `transforms`, `schemas`, and an operation's `examples`) are entry names, not fields — an `x-`-prefixed key there defines an ordinary entry named `x-…`, which enters the document's identifier namespace like any other key ([OBI-D-03](#142-document-rules), [OBI-D-04](#142-document-rules)), not an extension. Property names inside JSON Schema objects are schema content and follow [§6.2. Schemas](#62-schemas).
