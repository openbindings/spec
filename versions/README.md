...
Each snapshot captures the normative core at the time of release: the core spec, JSON Schema, editors list, and conformance suite. Companion format specs (under `formats/`) and the published interfaces (in [openbindings/interfaces](https://github.com/openbindings/interfaces)) are independently versioned and are **not** snapshotted; see [`RELEASING.md`](../RELEASING.md) for details.
